...
*/
extern u16 bt_get_curr_channel_state(void);
/*
u8 bt_get_call_status(); 与BT_CALL_IND_STA 枚举的值判断
用于获取当前蓝牙电话的状态
*/
extern u8 bt_get_call_status(void);
/*根据地址获取电话状态*/
...
/*当前连接的设备是不是jl测试盒*/
extern bool bt_get_remote_test_flag(void);
//查询当前蓝牙的状态
...
extern u8 bt_get_esco_coder_busy_flag(void);
...
/*提供接口外部设置要保留hfp_ag不要蓝牙通话*/
extern void bt_set_disable_hfp_ag_flag(bool flag);
...
//发射器获取当前连接链路状态接口
u16 bt_emitter_get_curr_channel_state(void);
//发射器获取当前音乐状态接口
...
extern u8 bt_a2dp_is_source_dev_null(void);
/****8发射器的一些相关接口end*********/
...
/*后台的恢复接口*/
void btstack_resume(void);//background resume
/*后台时退出蓝牙的处理接口*/
int btstack_suspend(void);

/*查询地址是不是杰理配套的蓝牙dongle*/
int btstack_get_dev_type_for_addr(u8 *addr);
/*查询当前有没有杰理配套的蓝牙dongle连接*/
bool bt_check_is_have_dongle_dev_conn(void);
bool bt_check_is_have_tws_share_dev_conn(void);
u8 *get_tws_share_dev_addr(void);
...
int btstack_get_num_of_remote_device_recorded(void);
/*根据记录的蓝牙库conn地址去操作*/
...
/*获取正在处于来电状态的设备数目*/
u8 bt_stack_get_incoming_call_num(void);
/*根据地址获取设备是否支持来电铃声*/
