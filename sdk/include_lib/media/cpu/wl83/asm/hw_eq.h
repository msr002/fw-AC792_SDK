...
void butterworth_lp_design(int fc, int fs, int nSOS, float *coeff);
...
coeff: 输出系数地址 （ 大小为((nSOS & 1) + (nSOS >> 1))*5 , 顺序是按照硬件EQ摆放了 )
段数：((nSOS & 1) + (nSOS >> 1))*/
void butterworth_hp_design(int fc, int fs, int nSOS, float *coeff);
...
*/
/*----------------------------------------------------------------------------*/
bool audio_hw_eq_is_running(struct hw_eq *eq);
