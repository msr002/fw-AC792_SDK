...
/*是否在重启*/
u8 get_aec_rebooting(void);

int sms_tde_init(struct aec_s_attr *attr);
int sms_tde_exit(void);
...
/*获取是否在重启中*/
u8 get_sms_tde_rebooting(void);
int cvp_sms_read_ref_data(void);
int cvp_sms_tde_read_ref_data(void);
/*可写长度*/
int get_cvp_sms_output_way_writable_len(void);
int get_cvp_sms_tde_output_way_writable_len(void);
