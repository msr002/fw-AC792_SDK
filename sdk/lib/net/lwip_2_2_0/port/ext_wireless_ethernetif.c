...
void ext_netdev_rx_register(void (*fun)(void *priv, void *data, unsigned int len), void *priv);
void *ext_netdev_alloc_output_buf(unsigned char **buf, unsigned int len);
int ext_netdev_send_data(void *priv, void *skb);
int netdev_get_mac_addr(char *mac_addr);
...
static void low_level_init(struct netif *netif)
{
    ...
    ext_netdev_rx_register((void (*)(void *, void *, unsigned int))ethernetif_input, netif);
...
static int send_packet(struct netif *netif, struct pbuf *p)
{
    ...
    buffer = (char *)ext_netdev_alloc_output_buf((unsigned char **)(&pos), p->tot_len);
    if (buffer == NULL) {
        return -1;
    }
...
//	printf("low_level_output len = %d \r\n",p->tot_len);
    ...
    ret = ext_netdev_send_data(netif->state, buffer);
 ...
 */
static struct pbuf *low_level_input(u8_t *rx_pkt, int len)
{
    struct pbuf *q, *p;
    int i;
 ...
 */
err_t ext_wireless_ethernetif_init(struct netif *netif)
{
