...
static int(*bt_ethernetif_send_callback)(u8 *addr, u32 cmd, u32 value, u8 *data);
...
static int send_packet(struct netif *netif, struct pbuf *p)
{
    ...
    if (!bt_ethernetif_send_callback) {
        return -1;
    }
    ...
    ret = bt_ethernetif_send_callback(NULL, 0xff, p->tot_len, buffer);
...
void set_bt_ethernetif_send_callback(int (*callback)(u8_t *, u32, u32, u8_t *))
{
    bt_ethernetif_send_callback = callback;
}
