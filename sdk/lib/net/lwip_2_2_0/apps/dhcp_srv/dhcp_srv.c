...
static struct ip4_addr dns_server_address;
...
//ios 上网
        ...
        *optptr++ = 4;  //len
        if (dns_server_address.addr) {
            *optptr++ = ip4_addr1(&dns_server_address);
            *optptr++ = ip4_addr2(&dns_server_address);
            *optptr++ = ip4_addr3(&dns_server_address);
            *optptr++ = ip4_addr4(&dns_server_address);
        } else {
            *optptr++ = ip4_addr1(&server_address);
            *optptr++ = ip4_addr2(&server_address);
            *optptr++ = ip4_addr3(&server_address);
...
#if DHCPS_UNICAST_ENABLE && ETHARP_SUPPORT_STATIC_ENTRIES
#define BOOTP_UNICAST 0x0
    m->flags = BOOTP_UNICAST;
    IP4_ADDR(&broadcast_dhcps, ip4_addr1(&cli->ipaddr), ip4_addr2(&cli->ipaddr), ip4_addr3(&cli->ipaddr), ip4_addr4(&cli->ipaddr));
    struct eth_addr ethaddr;
    memcpy(ethaddr.addr, cli->cli_mac, 6);
    etharp_add_static_entry((const ip4_addr_t *)&broadcast_dhcps, (struct eth_addr *)&ethaddr);
#endif
...
#if DHCPS_UNICAST_ENABLE && ETHARP_SUPPORT_STATIC_ENTRIES
            etharp_remove_static_entry((ip4_addr_t *)&cli->ipaddr);
#endif
...
void dhcps_init(u8_t lwip_netif)
{
...
void dhcps_set_dns_server(struct ip4_addr *ipaddr)
{
    memcpy(&dns_server_address.addr, &ipaddr->addr, sizeof(struct ip4_addr));
}
