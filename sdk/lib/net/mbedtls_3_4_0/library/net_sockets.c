...
void mbedtls_net_set_timeout(mbedtls_net_context *ctx, int send_to_ms, int recv_to_ms)
{
    ...
    if (ctx->hdl && ctx->send_to_ms) {
        sock_set_send_timeout((void *)ctx->hdl, ctx->send_to_ms);
    }
    if (ctx->hdl && ctx->recv_to_ms) {
...
static int mbedtls_bind(void *fd, const char *node, const char *port, int proto)
{
    ...
    if (getaddrinfo(node, port, &hints, &addr_list)) {
        printf("httpin_bind getaddrinfo err\n");
        return -1;
    }

    for (cur = addr_list; cur != NULL; cur = cur->ai_next) {
        if (sock_bind(fd, cur->ai_addr, cur->ai_addrlen) != 0) {
            ret = -1;
            continue;
        }
...
int mbedtls_net_send(void *ctx, const unsigned char *buf, size_t len)
{
    ...
    int ret = sock_send(fd, (const void *)buf, len, 0);
    if (ret < 0) {
        if (net_would_block(ctx) != 0) {
