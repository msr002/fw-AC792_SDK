...
static int compare_dhcp_ipaddr(void)
{
    ...
    if (ret < 0) {
        log_error("compare_dhcp_ipaddr NO VM_STA_IPADDR_INDEX");
        return -1;
    }
    ...
    log_error("compare_dhcp_ipaddr not Match!!! [%s][%s],[0x%x,0x%x][0x%x,0x%x],[0x%x]", info.ssid, sta_ip_info.ssid, local_mac[0], local_mac[5], sta_ip_info.local_mac[0], sta_ip_info.local_mac[5], sta_ip_info.dns);
