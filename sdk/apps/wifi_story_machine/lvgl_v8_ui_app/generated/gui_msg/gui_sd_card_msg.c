...
void gui_sd_card_msg_sdcard_not_found_label_set_text_cb(lv_event_t *e)
{
    lv_obj_t *obj = lv_event_get_target(e);
    if (guider_ui.sdcard_del || obj == NULL) {
        return;
    }
    lv_msg_t *msg = (lv_msg_t *)lv_event_get_param(e);
    if (msg == NULL || msg->id != GUI_SD_CARD_MSG_ID_SDCARD) {
        return;
    }
...
void gui_sd_card_msg_init_ui()
{
    if (!guider_ui.sdcard_del) {
        gui_msg_action_change(GUI_SD_CARD_MSG_ID_SDCARD, GUI_MSG_ACCESS_GET, &guider_msg_data, VALUE_STRING);
        lv_label_set_text(guider_ui.sdcard_not_found_label, guider_msg_data.value_string);
    }
}
...
void gui_sd_card_msg_init_events()
{
    ...
    char sub_ids[1] = {0};
    char unsub_ids[1] = {0};
    int32_t first_id = GUI_SD_CARD_MSG_ID_SDCARD;
    ...
    if (!guider_ui.sdcard_del) {
        lv_obj_remove_event_cb(guider_ui.sdcard_not_found_label, gui_sd_card_msg_sdcard_not_found_label_set_text_cb);
        lv_obj_add_event_cb(guider_ui.sdcard_not_found_label, gui_sd_card_msg_sdcard_not_found_label_set_text_cb, LV_EVENT_MSG_RECEIVED, NULL);

        res = lv_msg_subsribe_obj(GUI_SD_CARD_MSG_ID_SDCARD, guider_ui.sdcard_not_found_label, NULL);
        gui_msg_insert_list(&subs_ll, res);

        sub_ids[GUI_SD_CARD_MSG_ID_SDCARD - first_id] = 1;
    }
...
gui_msg_status_t gui_sd_card_msg_send(int32_t msg_id, void *value, int32_t len)
{
    if (msg_id == GUI_SD_CARD_MSG_ID) {
        lv_msg_send(GUI_SD_CARD_MSG_ID_SDCARD, NULL);
    } else {
