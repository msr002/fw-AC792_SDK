...
void gui_key_msg_key_show_label_set_text_cb(lv_event_t *e)
{
    lv_obj_t *obj = lv_event_get_target(e);
    if (guider_ui.key_del || obj == NULL) {
        return;
    }
    lv_msg_t *msg = (lv_msg_t *)lv_event_get_param(e);
    if (msg == NULL || msg->id != GUI_KEY_MSG_ID_KEY_TEXT) {
        return;
    }
...
{
    static char key_text_init_var[] = "Please press the key";
...
void gui_key_msg_init_ui()
{
    if (!guider_ui.key_del) {
        gui_msg_action_change(GUI_KEY_MSG_ID_KEY_TEXT, GUI_MSG_ACCESS_GET, &guider_msg_data, VALUE_STRING);
        lv_label_set_text(guider_ui.key_show_label, guider_msg_data.value_string);
    }
}
...
void gui_key_msg_init_events()
{
    ...
    char sub_ids[1] = {0};
    char unsub_ids[1] = {0};
    int32_t first_id = GUI_KEY_MSG_ID_KEY_TEXT;
    ...
    if (!guider_ui.key_del) {
        lv_obj_remove_event_cb(guider_ui.key_show_label, gui_key_msg_key_show_label_set_text_cb);
        lv_obj_add_event_cb(guider_ui.key_show_label, gui_key_msg_key_show_label_set_text_cb, LV_EVENT_MSG_RECEIVED, NULL);

        res = lv_msg_subsribe_obj(GUI_KEY_MSG_ID_KEY_TEXT, guider_ui.key_show_label, NULL);
        gui_msg_insert_list(&subs_ll, res);

        sub_ids[GUI_KEY_MSG_ID_KEY_TEXT - first_id] = 1;
    }
...
gui_msg_status_t gui_key_msg_send(int32_t msg_id, void *value, int32_t len)
{
    if (msg_id == GUI_KEY_MSG_ID) {
        lv_msg_send(GUI_KEY_MSG_ID_KEY_TEXT, NULL);
    } else {
