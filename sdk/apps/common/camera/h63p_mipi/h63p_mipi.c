...
#define FRAME_W  1920
#define FRAME_H30  750
#define FRAME_H25  900

#define H63P_FPS  25    //30
...
unsigned char wrH63P_MIPIReg(unsigned char regID, unsigned char regDat)
{
    ...
    if (dev_ioctl(iic, IOCTL_IIC_TX_BYTE_WITH_START_BIT, H63P_WRCMD)) {
        ret = 0;
        //  puts("1 err\n");
        goto __wend;
    }
...
unsigned char rdH63P_MIPIReg(unsigned char regID, unsigned char *regDat)
{
    ...
    if (dev_ioctl(iic, IOCTL_IIC_TX_BYTE_WITH_START_BIT, H63P_RDCMD)) {
        ret = 0;
        // puts("6\n");
...
void H63P_MIPI_config_SENSOR(u16 *width, u16 *height, u8 *format, u8 *frame_freq)
{
    ...
    for (i = 0; i < sizeof(H63P_MIPI_INI_REG) / sizeof(Sensor_reg_ini); i++) {
        ...
        rdH63P_MIPIReg(H63P_MIPI_INI_REG[i].addr, &pid);
        /* printf("%02x: %02x->%02x\n", H63P_MIPI_INI_REG[i].addr, H63P_MIPI_INI_REG[i].value, pid); */
    }
//    rdH63P_MIPIReg(0x6D, &pid);
//    pid |= 0x30;
//    wrH63P_MIPIReg(0x6D, pid);
...
s32 H63P_MIPI_ID_check(void)
{
    ...
    for (i = 0; i < 4; i++) {
        ...
        printf("Sensor H63P_MIPI PID %x %x %x %x\n", pid, ver, H63P_WRCMD, H63P_RDCMD);
...
static void H63P_MIPI_reset(u8 isp_dev)
{
...
s32 H63P_MIPI_check(u8 isp_dev, u32 reset_gpio, u32 pwdn_gpio, char *iic_name)
{
    printf("\n\nH63P_MIPI_check reset pin :%x\n\n", reset_gpio);
    if (!iic) {
...
REGISTER_CAMERA(H63P_MIPI) = {
    ...
    .isp_size 			= 	{H63P_MIPI_OUTPUT_W, H63P_MIPI_OUTPUT_H},

    .cap_fps                =   H63P_FPS,
    .sen_cap_size           =   {H63P_MIPI_OUTPUT_W, H63P_MIPI_OUTPUT_H},
    .isp_cap_size           =   {H63P_MIPI_OUTPUT_W, H63P_MIPI_OUTPUT_H},
