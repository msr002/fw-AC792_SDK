    ...
    while (1) {
        ...
        if (ret > 0) {
            ...
            if (jpeg_decode_image_info(&info)) {//获取JPEG图片信息
                ...
                ret = jpeg_decode_one_image(&req, JPEG_DEV_ID_0);//JPEG转YUV解码
                if (ret) {
...
{
    thread_fork("avi_unpkg_example", 10, 1500 * 3, 64, 0, avi_unpkg_decoder_to_yuv_example, NULL);
    return 0;
}
#endif
#endif
