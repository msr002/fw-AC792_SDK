...
uint32_t volc_timestamp_format_with_ms_and_timezone(char *p_dest_buffer, uint32_t dest_buffer_len, uint64_t timestamp_milliseconds)
{
    ...
    str_len = sprintf(p_dest_buffer, "%d-%02d-%02d %02d:%02d:%02d.%03lld ", timeinfo->tm_year, timeinfo->tm_mon, timeinfo->tm_mday,
...
#if 1
//    volc_debug("%s\n", __FUNCTION__);
//    printf("%s time:%d %s \n", __FUNCTION__, timestamp, p_dest_buffer);
...
//    printf("%s\n", __FUNCTION__);
    ...
    struct tm *nowtime = localtime(&timestamp_seconds);
//    printf("year:%d, mon:%d, mday:%d, hour:%d, min:%d, sec:%d", nowtime->tm_year, nowtime->tm_mon, nowtime->tm_mday, nowtime->tm_hour, nowtime->tm_min, nowtime->tm_sec);
    strftime(time_str, maxsize, "%Y%m%d%H%M%S", nowtime);
//    printf("maxsize %d, time_str:%s", maxsize, time_str);
err_out_label:
    return ret;
#endif // 0
}
