...
static int log_level_ = LLL_USER | LLL_ERR | LLL_WARN;
...
// add custom headers
    ...
    case LWS_CALLBACK_CLIENT_FILTER_PRE_ESTABLISH: {
        ...
        if (header_len > 0) {
            ...
            if (strcmp(content_type, "text/event-stream") == 0) {
...
//        printf("clent_data:%p", http_ctx->client_data);
//        printf(" clent_data->is_sse:%d", http_ctx->client_data->is_sse);
        // 处理 server sent event(SSE) 数据
    if (http_ctx->client_data->is_sse) {
//		    printf("\n -[function] %s -[line] %d\n", __FUNCTION__, __LINE__);
        lwsl_debug("begin to process sse data\n");
        sse_context_t *sse = http_ctx->client_data->sse_ctx;
//            printf("\n -[function] %s -[line] %d\n", __FUNCTION__, __LINE__);
        if (len + sse->used_len >= sizeof(sse->buffer)) {
            // 如果缓冲区已满，丢弃数据（避免溢出）
            printf("SSE buffer overflow; truncating data.\n");
            sse->used_len = 0;
            // 处理溢出的逻辑
            // if (sse->buffer[0]) {
            // free(sse->buffer);
            memset(sse->buffer, 0, sizeof(sse->buffer));
            // }
            sse->buffer[0] = '\0';
            return 0;
        }
        ...
        // 查找缓冲区中的所有完整事件块（以 \n\n 分隔）
        char *chunk_start = sse->buffer;
        char *chunk_end = NULL;
        // while ((chunk_end = strnstr(chunk_start, "\n\n", sse->used_len))) {
        // 	size_t chunk_len = chunk_end - chunk_start + 2;
        while ((chunk_end = onesdk_memmem(chunk_start, sse->used_len - (chunk_start - sse->buffer), "\n\n", 2))) {
            size_t chunk_len = (char *)chunk_end - chunk_start + 2;
...
//                    printf("sse:%d", sse);
                ...
                // printf("after on_get_sse_cb, sse->buffer: %s, sse->data: %s\n", sse->buffer, sse->data);
            }
            // 移动指针到下一个可能的事件块起始位置
            chunk_start = chunk_end + 2;
            // 检查 chunk_start 是否超出了缓冲区的边界
            if (chunk_start >= sse->buffer + sse->used_len) {
                // lwsl_debug("SSE chunk stats1: used_len: %zu  next chunk start = %p buffer start = %p.\n", sse->used_len, chunk_start ,sse->buffer);
                break;
            }
        }

        // 移动剩余数据到缓冲区起始位置
        // lwsl_debug("SSE chunk stats2: used_len: %zu next chunk start = %p, buffer start = %p \n", sse->used_len, chunk_start, sse->buffer);
        size_t remaining_len = sse->used_len - (chunk_start - sse->buffer);
        // lwsl_debug("SSE chunk stats3: remaining_len: %zu\n", remaining_len);
        memmove(sse->buffer, chunk_start, remaining_len);
        sse->used_len = remaining_len;
        // lwsl_debug("SSE chunk stats4: used_len: %zu next chunk start = %p, buffer start = %p. \n", sse->used_len, chunk_start, sse->buffer);
...
{
    lws_set_log_level(log_level_, NULL);
