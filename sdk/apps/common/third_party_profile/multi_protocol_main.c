...
#define SDP_RECORD_REGISTER(handler) \
	static const sdp_protocal_item_t handler SEC_USED(.sdp_record_item)
...
static const uint8_t rcsp_profile_data[] = {
    //////////////////////////////////////////////////////
    //
...
extern u8 get_bt_le_audio_config(void);
...
extern int mma_protocol_loop_process(void);
...
extern int dueros_process(void);
...
static const u8 sdp_honor_spp_service_data[96] = {
...
extern void le_hogp_all_init(void);
extern void le_hogp_all_exit(void);
#endif


static bool check_tws_master_role(void)
{
#if TCFG_USER_TWS_ENABLE
...
static void multi_protocol_loop_process(void *parm)
{
    ...
    while (1) {
        ...
        case MULTI_PROTOCOL_TASK_EXIT:
            ...
            break;
        default:
            log_error("err multi protocal msg");
            break;
        }
    }
}
...
#elif (THIRD_PARTY_PROTOCOLS_SEL & LE_HOGP_EN)
    /* app_ble_sm_init(IO_CAPABILITY_DISPLAY_ONLY, SM_AUTHREQ_BONDING | SM_AUTHREQ_MITM_PROTECTION, 7, 0); */
...
void multi_protocol_bt_init(void)
{
    ...
    extern int mijia_all_init(void);
    mijia_all_init();
#endif
...
#if (THIRD_PARTY_PROTOCOLS_SEL & MIJIA_EN)
    extern int mijia_all_exit(void);
    mijia_all_exit();
#endif
...
_WEAK_ void ble_profile_init(void)
{
}
