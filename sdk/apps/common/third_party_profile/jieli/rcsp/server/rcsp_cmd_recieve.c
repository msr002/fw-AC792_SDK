...
void rcsp_cmd_recieve(void *priv, u8 OpCode, u8 OpCode_SN, u8 *data, u16 len, u16 ble_con_handle, u8 *spp_remote_addr)
{
    ...
    case JL_OPCODE_FILE_TRANSFER_START:
        log_info("%s: JL_OPCODE_FILE_TRANSFER_START", __FUNCTION__);
...
#if (RCSP_MODE && (JL_RCSP_EXTRA_FLASH_OPT || RCSP_TONE_FILE_TRANSFER_ENABLE || (RCSP_MODE == RCSP_MODE_SOUNDBOX && RCSP_FILE_OPT)))
