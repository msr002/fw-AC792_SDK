...
#pragma code_seg(".ble_rcsp_multi_common.text")
#endif
/*********************************************************************************************
    *   Filename        : .c
...
#define CHANGE_TO_WATACH          1 //对某些和_rcsp_module.c冲突实行屏蔽
#define PASSKEY_ENTER_ENABLE      0 //输入passkey使能，可修改passkey
//----------------------------------------------------------------------------------------
#define RCSP_MULTI_ATT_MTU_SIZE              (517) //ATT MTU的值

//ATT发送的包长,    note: 23 <= need >= MTU
#define RCSP_MULTI_ATT_LOCAL_PAYLOAD_SIZE    (RCSP_MULTI_ATT_MTU_SIZE)                   //
//ATT缓存的buffer大小,  note: need >= 23,可修改
#define RCSP_MULTI_ATT_SEND_CBUF_SIZE        (1024*6+512)//(512*2)                 //

//共配置的RAM
#define RCSP_MULTI_ATT_RAM_BUFSIZE           (ATT_CTRL_BLOCK_SIZE + RCSP_MULTI_ATT_LOCAL_PAYLOAD_SIZE + RCSP_MULTI_ATT_SEND_CBUF_SIZE)                   //note:
static u8 rcsp_multi_att_ram_buffer[RCSP_MULTI_ATT_RAM_BUFSIZE] __attribute__((aligned(4)));
...
extern const int config_btctler_le_master_rcsp_multilink;
...
s8 mul_get_dev_index(u16 handle, u8 role)
{
    ...
    if (RCSP_MULTI_ROLE_SERVER == role) {
    ...
    return RCSP_MULTI_INVAIL_INDEX;
}
...
u8 mul_dev_get_handle_role(u16 handle)
{
    ...
    for (i = 0; i < SUPPORT_MAX_CLIENT; i++) {
        if (handle == client_con_handle[i]) {
            return RCSP_MULTI_ROLE_CLIENT;
        }
    }
    return RCSP_MULTI_ROLE_SERVER;
}
...
void multi_cbk_sm_packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size)
{
    ...
    case HCI_EVENT_PACKET:
        ...
        case SM_EVENT_PAIR_PROCESS:
            ...
            if (mul_dev_get_handle_role(event->con_handle) == RCSP_MULTI_ROLE_SERVER) {
                rcsp_trans_cbk_sm_packet_handler(packet_type, channel, packet, size);
            }
#endif

#if SUPPORT_MAX_CLIENT
            if (mul_dev_get_handle_role(event->con_handle) == RCSP_MULTI_ROLE_CLIENT) {
                rcsp_client_cbk_sm_packet_handler(packet_type, channel, packet, size);
            }
...
void multi_cbk_packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size)
{
    ...
    case HCI_EVENT_PACKET:
        ...
        case ATT_EVENT_CAN_SEND_NOW:
            if (0 == packet[1]) {
                role = RCSP_MULTI_ROLE_SERVER;
            } else {
                role = RCSP_MULTI_ROLE_CLIENT;
            }
            break;
    ...
    printf("ble_rcsp_multi_common-role:%d\n", role);
#if SUPPORT_MAX_SERVER
    if (role == 0xff || role == RCSP_MULTI_ROLE_SERVER) {
        rcsp_trans_cbk_packet_handler(packet_type, channel, packet, size);
    }
#endif

#if SUPPORT_MAX_CLIENT
    if (role == 0xff || role == RCSP_MULTI_ROLE_CLIENT) {
        rcsp_client_cbk_packet_handler(packet_type, channel, packet, size);
    }
#endif
...
{
#if SUPPORT_MAX_SERVER
    if (mul_dev_get_handle_role(conn_handle) == RCSP_MULTI_ROLE_SERVER) {
        rcsp_trans_deal_passkey_input(key, conn_handle);
    }
#endif

#if SUPPORT_MAX_CLIENT
    if (mul_dev_get_handle_role(conn_handle) == RCSP_MULTI_ROLE_CLIENT) {
        rcsp_client_deal_passkey_input(key, conn_handle);
    }

#endif
}

void rcsp_multi_ble_sm_setup_init(io_capability_t io_type, u8 auth_req, uint8_t min_key_size, u8 security_en)
{
...
#define RCSP_MULTI_TCFG_BLE_SECURITY_EN          0/*是否发请求加密命令*/
void rcsp_multi_ble_profile_init(u8 enable)
{
    ...
    if (enable) {
        ...
        ble_vendor_set_default_att_mtu(RCSP_MULTI_ATT_MTU_SIZE);

        if (config_le_sm_support_enable) {
            le_device_db_init();
#if PASSKEY_ENTER_ENABLE
            rcsp_multi_ble_sm_setup_init(IO_CAPABILITY_DISPLAY_ONLY, SM_AUTHREQ_MITM_PROTECTION | SM_AUTHREQ_BONDING, 7, RCSP_MULTI_TCFG_BLE_SECURITY_EN);
#else
            rcsp_multi_ble_sm_setup_init(IO_CAPABILITY_NO_INPUT_NO_OUTPUT, SM_AUTHREQ_MITM_PROTECTION | SM_AUTHREQ_BONDING, 7, RCSP_MULTI_TCFG_BLE_SECURITY_EN);
#endif
        }
        ...
        ble_op_multi_att_send_init(rcsp_multi_att_ram_buffer, RCSP_MULTI_ATT_RAM_BUFSIZE, RCSP_MULTI_ATT_LOCAL_PAYLOAD_SIZE);

#if SUPPORT_MAX_SERVER
        rcsp_bt_multi_trans_init();//从机初始化+打开adv
#endif

#if SUPPORT_MAX_CLIENT
        rcsp_bt_multi_client_init();
#endif
    } else {
        /* rcsp_multi_ble_module_enable(0);   */
    }
}
...
{
#if SUPPORT_MAX_SERVER
    ble_rcsp_multi_trans_disconnect();
#endif

#if SUPPORT_MAX_CLIENT
    ble_rcsp_multi_client_disconnect();
#endif
}
...
#if SUPPORT_MAX_SERVER
    rcsp_bt_multi_trans_init();
#endif

#if SUPPORT_MAX_CLIENT
    rcsp_bt_multi_client_init();
    if (config_btctler_le_master_rcsp_multilink) {
        //ble_vendor_set_hold_prio(0, 1);
    }
#endif
}

extern void bt_multi_trans_exit(void);
extern void bt_multi_client_exit(void);
