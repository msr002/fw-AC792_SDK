...
lv_slider_orientation_t lv_slider_get_orientation(lv_obj_t *slider)
{
    ...
    if (ori == LV_BAR_ORIENTATION_HORIZONTAL) {
        return LV_SLIDER_ORIENTATION_HORIZONTAL;
    } else if (ori == LV_BAR_ORIENTATION_VERTICAL) {
        return LV_SLIDER_ORIENTATION_VERTICAL;
    } else {
        ...
        *s  = LV_MAX(*s, knob_size);
        ...
        if (c == LV_KEY_RIGHT || c == LV_KEY_UP) {
            if (!slider->left_knob_focus) {
                ...
                lv_slider_set_start_value(obj, lv_slider_get_left_value(obj) + 1, LV_ANIM_ON);
            }
        ...
        if (!slider->left_knob_focus) {
            lv_slider_set_value(obj, lv_slider_get_value(obj) + r, LV_ANIM_ON);
        } else {
            lv_slider_set_start_value(obj, lv_slider_get_left_value(obj) + 1, LV_ANIM_ON);
        }
...
static bool is_slider_horizontal(lv_obj_t *obj)
{
    ...
    } else if (slider->bar.orientation == LV_BAR_ORIENTATION_HORIZONTAL) {
        return true;
    } else {
        return false;
    }
