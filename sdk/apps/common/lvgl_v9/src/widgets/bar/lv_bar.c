...
void lv_bar_set_range(lv_obj_t *obj, int32_t min, int32_t max)
{
    ...
    if (bar->min_value == real_min && bar->max_value == real_max) {
        return;
    }
...
static void draw_indic(lv_event_t *e)
{
    ...
    */
    ...
    if (bg_radius > short_side >> 1) {
        bg_radius = short_side >> 1;
    }
    ...
    if (indic_radius > short_side >> 1) {
        indic_radius = short_side >> 1;
    }
    ...
    if (hor && draw_rect_dsc.bg_grad.dir == LV_GRAD_DIR_HOR) {
        mask_needed = true;
    } else if (!hor && draw_rect_dsc.bg_grad.dir == LV_GRAD_DIR_VER) {
        mask_needed = true;
    }

    if (draw_rect_dsc.bg_image_src) {
        mask_needed = true;
    }

    bool radius_issue = true;
    /*The indicator is fully drawn if it's larger than the bg*/
    if ((bg_left < 0 || bg_right < 0 || bg_top < 0 || bg_bottom < 0)) {
        radius_issue = false;
    } else if (indic_radius >= bg_radius) {
        radius_issue = false;
    } else if (lv_area_is_in(&indic_area, &bar_coords, bg_radius)) {
        radius_issue = false;
    }
