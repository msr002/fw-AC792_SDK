...
static void analytics_create(lv_obj_t *parent)
{
    ...
    if (disp_size < DISP_LARGE) {
        lv_obj_add_flag(lv_obj_get_parent(scale2), LV_OBJ_FLAG_FLEX_IN_NEW_TRACK);
    }

    scale3 = create_scale_box(parent, "Network Speed", "Low speed", "Normal Speed", "High Speed");
    if (disp_size < DISP_LARGE) {
        lv_obj_add_flag(lv_obj_get_parent(scale3), LV_OBJ_FLAG_FLEX_IN_NEW_TRACK);
    }
...
static void slider_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
        ...
        *s = LV_MAX(*s, 60);
        ...
        if (draw_task == NULL || lv_draw_task_get_type(draw_task) != LV_DRAW_TASK_TYPE_FILL) {
            return;
        }
