...
static void lcd_800x480_fl58202_backctrl(struct lcd_board_cfg *bd_cfg, u8 on)
{
...
static int lcd_800x480_fl58202_init(struct lcd_board_cfg *bd_cfg)
{
    printf("lcd 800x480_fl58202 init ...\n");

    return 0;
}

static int lcd_800x480_fl58202_check_id(struct lcd_board_cfg *bd_cfg)
{
    // TODO: check id
...
REGISTER_IMD_DEVICE_BEGIN(lcd_800x480_fl58202_dev) = {
    .info = {
#ifdef USE_TEST_MODE
        .test_mode       = true,
#else
...
#define  HBP          18 //行同步信号后肩 单位：clk
#define  HFP          8  //行同步信号前肩 单位：clk
#define  HSPW         4  //水平信号宽度   单位：clk
#define  VSPW         4  //垂直信号宽度   单位：行
#define  VBP          8  //帧同步信号后肩 单位：行
#define  VFP          8  //帧同步信号前肩 单位：行

        //以下信号均为高有效1 以下为固定公式勿动
        .hori_total           = __LCD_W * 1 + HBP + HFP + HSPW, //行总长 : 行像素*3 + 行同步信号后肩 +
        //                                                                 行同步信号前肩 + 水平信号宽度
        .hori_sync            = __LCD_W * 1 + HBP + HFP,        //行同步时间:  行像素*3 + 行同步信号后肩 + 行同步信号前肩
        .hori_back_porth      = HBP,                            //行同步信号后肩 (+HSPW?)
        .vert_total           = VSPW + VBP + __LCD_H + VFP,     //帧总长: 垂直信号宽度 + 帧同步信号后肩 +
        //                                                                像素高 + 帧同步信号后肩
        .vert_sync            = VSPW,                           //帧于帧之间间隔多少个行中断?
        .vert_back_porth_odd  = VBP + VSPW,                     //帧信号开始的时候经过多少个行中断开始de:
        //                                                        帧同步信号后肩 + 水平信号宽度
...
REGISTER_LCD_DEVICE_DRIVE(lcd_dev)  = {
    .logo            = "RGB_800X480_FL58202",
    .id              = __LCD_ID,
    .type            = LCD_RGB,
    .dev             = &lcd_800x480_fl58202_dev,
    .init            = lcd_800x480_fl58202_init,
    .bl_ctrl         = lcd_800x480_fl58202_backctrl,
    .check           = lcd_800x480_fl58202_check_id,
    .send_init_code  = NULL,
};
