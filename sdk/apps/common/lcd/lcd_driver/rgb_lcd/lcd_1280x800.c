...
static void lcd_1280x800_backctrl(void *_data, u8 on)
{
...
static int lcd_1280x800_init(void *_data)
{
    printf("lcd 1280x800 init ...\n");

    return 0;
}


REGISTER_IMD_DEVICE_BEGIN(lcd_1280x800_dev) = {
    .info = {
#ifdef USE_TEST_MODE
        .test_mode 	     = true,
#else
    ...
    .dclk_cfg       = DPI_CLK_PLL_192M | DIVA_3 | DIVB_1,

    .timing = {

#define  HBP                    88    //行同步信号后肩 单位：clk
#define  HFP                    72    //行同步信号前肩 单位：clk
#define  HSPW                   10    //水平信号宽度   单位：clk
#define  VSPW                   10    //垂直信号宽度   单位：行
#define  VBP                    23    //帧同步信号后肩 单位：行
#define  VFP                    15    //帧同步信号前肩 单位：行
...
REGISTER_LCD_DEVICE_DRIVE(lcd_dev)  = {
    .logo       = "lcd_1280x800",
    .type		= LCD_RGB,
    .init		= lcd_1280x800_init,
    .dev    	= &lcd_1280x800_dev,
    .bl_ctrl	= lcd_1280x800_backctrl,
};
