...
#if (TCFG_BLE_DEMO_SELECT == DEF_BLE_DEMO_NET_CFG_TY)
...
static void cbk_packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size)
{
    ...
    case HCI_EVENT_PACKET:
        ...
        case ATT_EVENT_HANDLE_VALUE_INDICATION_COMPLETE:
            printf("ATT_EVENT_HANDLE_VALUE_INDICATION_COMPLETE\n");
        ...
        case HCI_EVENT_LE_META:
            ...
            case HCI_SUBEVENT_LE_CONNECTION_COMPLETE:
                ...
                server_profile_start(con_handle);
                /** 涂鸦ble连接状态*/
                extern void tuya_bt_state_connected(void);
                tuya_bt_state_connected();
                break;
            ...
            case HCI_SUBEVENT_LE_DATA_LENGTH_CHANGE:
                printf("APP HCI_SUBEVENT_LE_DATA_LENGTH_CHANGE\n");
                /* set_connection_data_phy(CONN_SET_CODED_PHY, CONN_SET_CODED_PHY); */
                break;

            case HCI_SUBEVENT_LE_PHY_UPDATE_COMPLETE:
                printf("APP HCI_SUBEVENT_LE_PHY_UPDATE %s\n", hci_event_le_meta_get_phy_update_complete_status(packet) ? "Fail" : "Succ");
        ...
        case HCI_EVENT_DISCONNECTION_COMPLETE:
            ...
            /** 涂鸦ble断开状态*/
            extern void tuya_bt_state_disconnected(void);
            tuya_bt_state_disconnected();
            break;
        ...
        case HCI_EVENT_VENDOR_REMOTE_TEST:
            printf("--- HCI_EVENT_VENDOR_REMOTE_TEST\n");
            break;
...
extern void tuya_ble_write_callback(unsigned short uuid, unsigned char *buf, unsigned short buf_size);
static int att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)//接收响应
{
    int result = 0;
    u16 tmp16;
    ...
    case ATT_CHARACTERISTIC_00000001_0000_1001_8001_00805F9B07D0_01_VALUE_HANDLE :
        puts("ATT_CHARACTERISTIC_00000001_0000_1001_8001_00805F9B07D0_01_VALUE_HANDLE");
        tuya_ble_write_callback(ATT_CHARACTERISTIC_00000001_0000_1001_8001_00805F9B07D0_01_VALUE_HANDLE, buffer, buffer_size);
        break;

    case ATT_CHARACTERISTIC_00000002_0000_1001_8001_00805F9B07D0_01_CLIENT_CONFIGURATION_HANDLE :
        puts("ATT_CHARACTERISTIC_00000002_0000_1001_8001_00805F9B07D0_01_CLIENT_CONFIGURATION_HANDLE");
...
//------------------------------------------------------
int tuya_set_adv_data(unsigned char *data, unsigned int len)
{
    if (NULL == data || 0 == len) {
        printf("tuya_set_adv_data : PARAM ERROR!");
        return -1;
    }

    if (len > ADV_RSP_PACKET_MAX) {
        puts("tuya_set_adv_data : adv_data overflow!\n");
        return -1;
    }

    adv_data_len = len;
    ble_op_set_adv_data(len, data);
    return 0;
}

int tuya_set_rsp_data(unsigned char *data, unsigned int len)
{
    if (NULL == data || 0 == len) {
        printf("tuya_set_rsp_data : PARAM ERROR!");
        return -1;
    }

    if (len > ADV_RSP_PACKET_MAX) {
        puts("tuya_set_rsp_data : rsp_data overflow!\n");
        return -1;
    }

    scan_rsp_data_len = len;
    ble_op_set_rsp_data(len, data);
    return 0;
}
...
static void advertisements_setup_init()
{
    printf("advertisements_setup_init");
...
static bool not_allowed_init = false;
static char edr1_name[32];
static const char *bt_get_local_name1(void)
{
#if defined(ZAPP_CFG_CAMX_RUN_ENABLE) && (ZAPP_CFG_CAMX_RUN_ENABLE == 1)
    extern char g_camx_netcfg_ble_name[64];
    printf("------------`bt_get_local_name`, %s", g_camx_netcfg_ble_name);
    return g_camx_netcfg_ble_name;
#elif defined (CONFIG_TANGE_ENABLE)
    const u8 *mac_addr;
    ...
    const u8 *mac_addr;
    mac_addr = bt_get_mac_addr();
    sprintf(edr1_name, "LLM-DEMO-TY-%02X%02X", mac_addr[4], mac_addr[5]);
#endif

//    const u8 *mac_addr;
//    mac_addr = bt_get_mac_addr();
//    printf("mac_addr[0]:%x, mac_addr[1]%x", mac_addr[0], mac_addr[1]);
//    sprintf(edr1_name, "LLM_IPCB_00%02X%02X", mac_addr[0], mac_addr[1]);
    return edr1_name;
//    return edr_name;
...
void bt_ble_init(void)
{
    ...
    memcpy(&gap_device_name[gap_device_name_len], "TUYA", ext_name_len);
...
void bt_ble_not_allowed_init(void)
{
    not_allowed_init = true;
}
...
void tuya_bt_send_data(uint16_t uuid, const unsigned char *data, const unsigned char len)  //发送数据给手机
{
    app_send_user_data(ATT_CHARACTERISTIC_00000002_0000_1001_8001_00805F9B07D0_01_VALUE_HANDLE, data, len, ATT_OP_AUTO_READ_CCC);
}
#endif
