...
#if (TCFG_BLE_DEMO_SELECT == DEF_BLE_DEMO_NET_CFG_XC)
#include "le_net_cfg_xc.h"
...
void xciot_ble_netcfg_send_data(const unsigned char *data, int len);
...
static int att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    case ATT_CHARACTERISTIC_865300b3_43e6_47b7_9cb0_5fc21d4ae340_01_VALUE_HANDLE:
        log_info("ATT_CHARACTERISTIC_865300b3_43e6_47b7_9cb0_5fc21d4ae340_01_VALUE_HANDLE rx");
        log_info_hexdump(buffer, buffer_size);
#if defined(ZAPP_CFG_CAMX_RUN_ENABLE) && (ZAPP_CFG_CAMX_RUN_ENABLE == 1)
        extern int camx_netcfg_ble_on_data(unsigned char *data, int len);
        camx_netcfg_ble_on_data(buffer, buffer_size);
#else /* ZAPP_CFG_CAMX_RUN_ENABLE */
        extern int tg_netcfg_ble_on_data(unsigned char *data, int len);
        tg_netcfg_ble_on_data(buffer, buffer_size);
#endif
        // echo back test
        //xciot_ble_netcfg_send_data(buffer, buffer_size);
        break;

    case ATT_CHARACTERISTIC_865300b2_43e6_47b7_9cb0_5fc21d4ae340_01_CLIENT_CONFIGURATION_HANDLE:
        puts("ATT_CHARACTERISTIC_865300b2_43e6_47b7_9cb0_5fc21d4ae340_01_CLIENT_CONFIGURATION_HANDLE");
...
static const char *bt_get_local_name(void)
{
#if defined(ZAPP_CFG_CAMX_RUN_ENABLE) && (ZAPP_CFG_CAMX_RUN_ENABLE == 1)
...
void bt_ble_init(void)
{
    ...
    char name[32];
    sprintf(name, "%s_%d", bt_get_local_name(), KEY_PRIFILE);
    /* sprintf(name, "%s", bt_get_local_name()); */
    gap_device_name_len = strlen(name);
    memcpy(gap_device_name, name, gap_device_name_len);
...
void xciot_ble_netcfg_send_data(const unsigned char *data, int len)
{
    app_send_user_data(ATT_CHARACTERISTIC_865300b2_43e6_47b7_9cb0_5fc21d4ae340_01_VALUE_HANDLE, data, len, ATT_OP_AUTO_READ_CCC);
}
#endif
