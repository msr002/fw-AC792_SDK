...
static void multiple_labels_cb(void)
{
    ...
    if (hor_res * ver_res > 800 * 480) {
        lv_obj_set_style_text_font(scr, &lv_font_benchmark_montserrat_26_aligned, 0);
    } else if (hor_res * ver_res > 320 * 240) {
        lv_obj_set_style_text_font(scr, &lv_font_benchmark_montserrat_20_aligned, 0);
    } else {
        lv_obj_set_style_text_font(scr, &lv_font_benchmark_montserrat_14_aligned, 0);
    }
#else
    if (hor_res * ver_res > 800 * 480) {
        lv_obj_set_style_text_font(scr, &lv_font_montserrat_26, 0);
    } else if (hor_res * ver_res > 320 * 240) {
...
static void containers_with_scrolling_cb(void)
{
    ...
    if (ver_cnt < 20) {
        ver_cnt = 20;    /*The test with many widgets*/
    }
...
static void table_draw_task_event_cb(lv_event_t *e)
{
    ...
    if (draw_dsc_base->part != LV_PART_ITEMS) {
        return;
    }
