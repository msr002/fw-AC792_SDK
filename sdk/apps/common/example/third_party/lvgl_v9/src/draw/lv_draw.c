...
void lv_draw_deinit(void)
{
    ...
    while (u) {
        ...
        if (cur_unit->delete_cb) {
            cur_unit->delete_cb(cur_unit);
        }
        lv_free(cur_unit);
    }
...
bool lv_draw_dispatch_layer(lv_display_t *disp, lv_layer_t *layer)
{
    ...
    while (t) {
        ...
        if (t->state == LV_DRAW_TASK_STATE_READY) {
            if (t_prev) {
                t_prev->next = t->next;    /*Remove it by assigning the next task to the previous*/
            } else {
                layer->draw_task_head = t_next;    /*If it was the head, set the next as head*/
            }
    ...
    /*Assign draw tasks to the draw_units*/
        ...
        while (u) {
            ...
            if (taken_cnt != LV_DRAW_UNIT_IDLE) {
                task_dispatched = true;
            }
            u = u->next;
        }
