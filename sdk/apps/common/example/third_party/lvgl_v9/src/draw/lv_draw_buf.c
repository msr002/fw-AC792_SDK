...
uint32_t lv_draw_buf_width_to_stride_ex(const lv_draw_buf_handlers_t *handlers, uint32_t w,
                                        lv_color_format_t color_format)
{
    if (handlers->width_to_stride_cb) {
        return handlers->width_to_stride_cb(w, color_format);
    } else {
        return 0;
    }
}
...
{
    if (handlers->align_pointer_cb) {
        return handlers->align_pointer_cb(data, color_format);
    } else {
        return NULL;
    }
}
                      ...
                      const lv_draw_buf_t *src, const lv_area_t *src_area)
{
    ...
    if (dest_area == NULL) {
        line_width = dest->header.w;
    } else {
        line_width = lv_area_get_width(dest_area);
    }
...
lv_result_t lv_draw_buf_adjust_stride(lv_draw_buf_t *src, uint32_t stride)
{
    ...
    /*Use global stride*/
    if (stride == 0) {
        stride = lv_draw_buf_width_to_stride(w, header->cf);
    }
...
static void *draw_buf_malloc(const lv_draw_buf_handlers_t *handlers, size_t size_bytes,
                             lv_color_format_t color_format)
{
    if (handlers->buf_malloc_cb) {
        return handlers->buf_malloc_cb(size_bytes, color_format);
    } else {
        return NULL;
    }
}
