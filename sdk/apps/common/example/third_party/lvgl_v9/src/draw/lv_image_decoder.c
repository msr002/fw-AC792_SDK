...
static lv_image_decoder_t *image_decoder_get_info(lv_image_decoder_dsc_t *dsc, lv_image_header_t *header)
{
    ...
    if (src_type == LV_IMAGE_SRC_FILE) {
        LV_LOG_TRACE("Try to find decoder for %s", (const char *)src);
    } else {
        LV_LOG_TRACE("Try to find decoder for %p", src);
    }

    lv_image_decoder_t *decoder;
    bool is_header_cache_enabled = lv_image_header_cache_is_enabled();

    if (is_header_cache_enabled && (src_type == LV_IMAGE_SRC_FILE || src_type == LV_IMAGE_SRC_BIN)) {
    ...
    LV_LL_READ(img_decoder_ll_p, decoder) {
        ...
        if (decoder->info_cb && decoder->open_cb) {
            ...
            if (decoder_prev) {
                LV_LOG_TRACE("Can't open image with decoder %s. Trying next decoder.", decoder_prev->name);
            }
    ...
    if (decoder == NULL) {
        LV_LOG_TRACE("No decoder found");
    } else {
        LV_LOG_TRACE("Found decoder %s", decoder->name);
    }
    ...
    if (is_header_cache_enabled && (src_type == LV_IMAGE_SRC_FILE || src_type == LV_IMAGE_SRC_BIN) && decoder) {
        ...
        if (entry == NULL) {
            if (src_type == LV_IMAGE_SRC_FILE || src_type == LV_IMAGE_SRC_BIN) {
                lv_free((void *)search_key.src);
            }
            return NULL;
        }
...
static lv_result_t try_cache(lv_image_decoder_dsc_t *dsc)
{
    ...
    if (entry) {
        ...
        dsc->user_data = cached_data->user_data;
