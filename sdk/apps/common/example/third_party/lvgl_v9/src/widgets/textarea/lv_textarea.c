...
void lv_textarea_add_char(lv_obj_t *obj, uint32_t c)
{
    ...
    if (ta->pwd_mode) {
        pwd_char_hider(obj);    /*Make sure all the current text contains only '*'*/
    }
...
void lv_textarea_delete_char_forward(lv_obj_t *obj)
{
    ...
    if (cp != lv_textarea_get_cursor_pos(obj)) {
...
void lv_textarea_set_cursor_pos(lv_obj_t *obj, int32_t pos)
{
    ...
    lv_textarea_t *ta = (lv_textarea_t *)obj;
    if ((uint32_t)ta->cursor.pos == (uint32_t)pos) {
        return;
    }
    ...
    if (pos > (int32_t)len || pos == LV_TEXTAREA_CURSOR_LAST) {
        pos = len;
    }
...
static bool char_is_accepted(lv_obj_t *obj, uint32_t c)
{
    ...
    if (ta->accepted_chars == NULL || ta->accepted_chars[0] == '\0') {
        return true;
    }
    /*Accepted character?*/
    uint32_t i = 0;
...
static void update_cursor_position_on_click(lv_event_t *e)
{
    ...
    if (point_act.x < 0 || point_act.y < 0) {
        return;    /*Ignore event from keypad*/
    }
    lv_point_t rel_pos;
...
static void draw_placeholder(lv_event_t *e)
{
    ...
    if (txt[0] == '\0' && ta->placeholder_txt && ta->placeholder_txt[0] != 0) {
        ...
        if (ta->one_line) {
            ph_dsc.flag |= LV_TEXT_FLAG_EXPAND;
        }
