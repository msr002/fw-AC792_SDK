...
void lv_indev_read(lv_indev_t *indev)
{
    ...
    if (indev->disp == NULL) {
        return;    /*Not assigned to any displays*/
    }
...
static void indev_keypad_proc(lv_indev_t *i, lv_indev_data_t *data)
{
    if (data->state == LV_INDEV_STATE_PRESSED && i->wait_until_release) {
        return;
    }
    ...
    if (data->state == LV_INDEV_STATE_PRESSED && prev_state == LV_INDEV_STATE_RELEASED) {
        ...
        /*Move the focus on PREV*/
            ...
            if (data->key == LV_KEY_ENTER) {
                ...
                if (send_event(LV_EVENT_PRESSED, indev_act) == LV_RESULT_INVALID) {
                    return;
                }
                ...
                if (send_event(LV_EVENT_CANCEL, indev_act) == LV_RESULT_INVALID) {
                    return;
                }
            }
    ...
    /*Pressing*/
        ...
        if (data->key == LV_KEY_ENTER) {
            if (send_event(LV_EVENT_PRESSING, indev_act) == LV_RESULT_INVALID) {
                return;
            }
        }
        ...
        if (i->long_pr_sent == 0 && lv_tick_elaps(i->pr_timestamp) > i->long_press_time) {
            ...
            if (data->key == LV_KEY_ENTER) {
                ...
                if (send_event(LV_EVENT_LONG_PRESSED, indev_act) == LV_RESULT_INVALID) {
                    return;
                }
            }
        }
        /*Long press repeated time has elapsed?*/
            ...
            if (data->key == LV_KEY_ENTER) {
                if (send_event(LV_EVENT_LONG_PRESSED_REPEAT, indev_act) == LV_RESULT_INVALID) {
                    return;
                }
            }
    ...
    /*Release happened*/
        ...
        if (data->key == LV_KEY_ENTER) {
            ...
            if (send_event(LV_EVENT_RELEASED, indev_act) == LV_RESULT_INVALID) {
                return;
            }

            if (i->long_pr_sent == 0) {
                if (send_event(LV_EVENT_SHORT_CLICKED, indev_act) == LV_RESULT_INVALID) {
                    return;
                }
            }

            if (send_event(LV_EVENT_CLICKED, indev_act) == LV_RESULT_INVALID) {
                return;
            }
    ...
    /*Pressing*/
        ...
        if (i->long_pr_sent == 0 && lv_tick_elaps(i->pr_timestamp) > i->long_press_time) {
            ...
            if (data->key == LV_KEY_ENTER) {
                ...
                if (indev_reset_check(indev_act)) {
                    return;
                }
        ...
        if (indev_obj_act != NULL) {
            ...
            if (is_enabled) {
                if (indev->pointer.last_hovered != indev_obj_act) {
                    if (send_event(LV_EVENT_HOVER_OVER, indev_act) == LV_RESULT_INVALID) {
                        return;
                    }
                }
                if (send_event(LV_EVENT_PRESSED, indev_act) == LV_RESULT_INVALID) {
            ...
            if (indev_act->wait_until_release) {
                return;
            }
