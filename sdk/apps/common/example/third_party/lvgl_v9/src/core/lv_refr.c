...
void lv_refr_now(lv_display_t *disp)
{
    ...
    if (disp) {
        ...
        while (d) {
            if (d->refr_timer) {
                lv_display_refr_timer(d->refr_timer);
            }
...
static lv_obj_t *lv_refr_get_top_obj(const lv_area_t *area_p, lv_obj_t *obj)
{
    ...
    if (lv_area_is_in(area_p, &obj->coords, 0) == false) {
        return NULL;
    }
...
static void refr_obj_and_children(lv_layer_t *layer, lv_obj_t *top_obj)
{
    ...
    if (top_obj == NULL) {
        top_obj = lv_display_get_screen_active(disp_refr);
    }
    if (top_obj == NULL) {
...
static bool alpha_test_area_on_obj(lv_obj_t *obj, const lv_area_t *area)
{
    ...
    if (!lv_area_is_on(area, &obj->coords)) {
        return true;
    }
...
static void refr_obj(lv_layer_t *layer, lv_obj_t *obj)
{
    ...
    if (layer_type == LV_LAYER_TYPE_NONE) {
        ...
        while (layer_area_act.y2 < layer_area_full.y2) {
            ...
            if (layer_area_act.y2 > layer_area_full.y2) {
                layer_area_act.y2 = layer_area_full.y2;
            }
            ...
            while (layer_draw_dsc.rotation > 3600) {
                layer_draw_dsc.rotation -= 3600;
            }
            while (layer_draw_dsc.rotation < 0) {
                layer_draw_dsc.rotation += 3600;
            }
