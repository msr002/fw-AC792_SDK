    ...
    while (i <  obj->style_cnt) {
        ...
        if (obj->styles[i].is_local || obj->styles[i].is_trans) {
            if (obj->styles[i].style) {
                lv_style_reset((lv_style_t *)obj->styles[i].style);
            }
                                    ...
                                    const lv_obj_style_transition_dsc_t *tr_dsc)
{
    ...
    if (v1.ptr == v2.ptr && v1.num == v2.num && lv_color_eq(v1.color, v2.color)) {
        return;
    }
    obj->state = prev_state;
    ...
    if (tr_dsc->prop == LV_STYLE_RADIUS) {
        if (v1.num == LV_RADIUS_CIRCLE || v2.num == LV_RADIUS_CIRCLE) {
            ...
            if (v1.num == LV_RADIUS_CIRCLE) {
                v1.num = LV_MIN(whalf + 1, hhalf + 1);
            }
            if (v2.num == LV_RADIUS_CIRCLE) {
                v2.num = LV_MIN(whalf + 1, hhalf + 1);
            }
        }
    }
...
lv_style_value_t lv_obj_style_apply_color_filter(const lv_obj_t *obj, lv_part_t part, lv_style_value_t v)
{
    ...
    if (f && f->filter_cb) {
        ...
        if (f_opa != 0) {
            v.color = f->filter_cb(f, v.color, f_opa);
        }
    }
    return v;
}
...
lv_style_state_cmp_t lv_obj_style_state_compare(lv_obj_t *obj, lv_state_t state1, lv_state_t state2)
{
    ...
    for (i = 0; i < obj->style_cnt; i++) {
        ...
        if (valid1 != valid2) {
            ...
            if (lv_style_get_prop(style, LV_STYLE_PAD_TOP, &v)) {
                layout_diff = true;
            } else if (lv_style_get_prop(style, LV_STYLE_PAD_BOTTOM, &v)) {
                layout_diff = true;
            } else if (lv_style_get_prop(style, LV_STYLE_PAD_LEFT, &v)) {
                layout_diff = true;
            } else if (lv_style_get_prop(style, LV_STYLE_PAD_RIGHT, &v)) {
                layout_diff = true;
            } else if (lv_style_get_prop(style, LV_STYLE_PAD_COLUMN, &v)) {
                layout_diff = true;
            } else if (lv_style_get_prop(style, LV_STYLE_PAD_ROW, &v)) {
                layout_diff = true;
            } else if (lv_style_get_prop(style, LV_STYLE_LAYOUT, &v)) {
                layout_diff = true;
            } else if (lv_style_get_prop(style, LV_STYLE_TRANSLATE_X, &v)) {
                layout_diff = true;
            } else if (lv_style_get_prop(style, LV_STYLE_TRANSLATE_Y, &v)) {
                layout_diff = true;
            } else if (lv_style_get_prop(style, LV_STYLE_WIDTH, &v)) {
                layout_diff = true;
            } else if (lv_style_get_prop(style, LV_STYLE_HEIGHT, &v)) {
                layout_diff = true;
            } else if (lv_style_get_prop(style, LV_STYLE_MIN_WIDTH, &v)) {
                layout_diff = true;
            } else if (lv_style_get_prop(style, LV_STYLE_MAX_WIDTH, &v)) {
                layout_diff = true;
            } else if (lv_style_get_prop(style, LV_STYLE_MIN_HEIGHT, &v)) {
                layout_diff = true;
            } else if (lv_style_get_prop(style, LV_STYLE_MAX_HEIGHT, &v)) {
                layout_diff = true;
            } else if (lv_style_get_prop(style, LV_STYLE_BORDER_WIDTH, &v)) {
                layout_diff = true;
            }
            ...
            if (lv_style_get_prop(style, LV_STYLE_TRANSFORM_WIDTH, &v)) {
                res = LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
            } else if (lv_style_get_prop(style, LV_STYLE_TRANSFORM_HEIGHT, &v)) {
                res = LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
            } else if (lv_style_get_prop(style, LV_STYLE_TRANSFORM_ROTATION, &v)) {
                res = LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
            } else if (lv_style_get_prop(style, LV_STYLE_TRANSFORM_SCALE_X, &v)) {
                res = LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
            } else if (lv_style_get_prop(style, LV_STYLE_TRANSFORM_SCALE_Y, &v)) {
                res = LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
            } else if (lv_style_get_prop(style, LV_STYLE_OUTLINE_OPA, &v)) {
                res = LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
            } else if (lv_style_get_prop(style, LV_STYLE_OUTLINE_PAD, &v)) {
                res = LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
            } else if (lv_style_get_prop(style, LV_STYLE_OUTLINE_WIDTH, &v)) {
                res = LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
            } else if (lv_style_get_prop(style, LV_STYLE_SHADOW_WIDTH, &v)) {
                res = LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
            } else if (lv_style_get_prop(style, LV_STYLE_SHADOW_OPA, &v)) {
                res = LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
            } else if (lv_style_get_prop(style, LV_STYLE_SHADOW_OFFSET_X, &v)) {
                res = LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
            } else if (lv_style_get_prop(style, LV_STYLE_SHADOW_OFFSET_Y, &v)) {
                res = LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
            } else if (lv_style_get_prop(style, LV_STYLE_SHADOW_SPREAD, &v)) {
                res = LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
            } else if (lv_style_get_prop(style, LV_STYLE_LINE_WIDTH, &v)) {
                res = LV_STYLE_STATE_CMP_DIFF_DRAW_PAD;
            } else if (res == LV_STYLE_STATE_CMP_SAME) {
                res = LV_STYLE_STATE_CMP_DIFF_REDRAW;
            }
        }
    }
...
static lv_style_t *get_local_style(lv_obj_t *obj, lv_style_selector_t selector)
{
    ...
    for (i = obj->style_cnt - 1; i > 0 ; i--) {
        ...
        if (obj->styles[i - 1].is_local || obj->styles[i - 1].is_trans) {
            break;
        }
...
static lv_obj_style_t *get_trans_style(lv_obj_t *obj,  lv_style_selector_t selector)
{
    uint32_t i;
    for (i = 0; i < obj->style_cnt; i++) {
        if (obj->styles[i].is_trans && obj->styles[i].selector == selector) {
            break;
        }
    }
                                    ...
                                    lv_style_value_t *v)
{
    ...
    for (i = 0; i < obj->style_cnt; i++) {
        ...
        if ((obj_style->style->has_group & group) == 0) {
            continue;
        }
    ...
    for (; i < obj->style_cnt; i++) {
        if ((obj->styles[i].style->has_group & group) == 0) {
            continue;
        }
        lv_obj_style_t *obj_style = &obj->styles[i];
        lv_part_t part_act = lv_obj_style_get_selector_part(obj->styles[i].selector);
...
static void trans_anim_cb(void *_tr, int32_t v)
{
    ...
    uint32_t i;
    for (i = 0; i < obj->style_cnt; i++) {
        if (obj->styles[i].is_trans == 0 || obj->styles[i].selector != tr->selector) {
            continue;
        }
        ...
        case LV_STYLE_BLEND_MODE:
            if (v < 255) {
                ...
                value_final.num = tr->end_value.num;
            }
            break;
        case LV_STYLE_TRANSITION:
        case LV_STYLE_TEXT_FONT:
            if (v < 255) {
                value_final.ptr = tr->start_value.ptr;
            } else {
                value_final.ptr = tr->end_value.ptr;
            }
            break;
        case LV_STYLE_COLOR_FILTER_DSC:
            if (tr->start_value.ptr == NULL) {
                value_final.ptr = tr->end_value.ptr;
            } else if (tr->end_value.ptr == NULL) {
                value_final.ptr = tr->start_value.ptr;
            } else if (v < 128) {
                value_final.ptr = tr->start_value.ptr;
            } else {
        ...
        case LV_STYLE_IMAGE_RECOLOR:
            if (v <= 0) {
                value_final.color = tr->start_value.color;
            } else if (v >= 255) {
        ...
        default:
            if (v == 0) {
                ...
                value_final.num = tr->start_value.num + ((int32_t)((int32_t)(tr->end_value.num - tr->start_value.num) * v) >> 8);
            }
            break;
        }
...
static lv_layer_type_t calculate_layer_type(lv_obj_t *obj)
{
    if (lv_obj_get_style_transform_rotation(obj, 0) != 0) {
        return LV_LAYER_TYPE_TRANSFORM;
    }
    if (lv_obj_get_style_transform_scale_x(obj, 0) != 256) {
        return LV_LAYER_TYPE_TRANSFORM;
    }
    if (lv_obj_get_style_transform_scale_y(obj, 0) != 256) {
        return LV_LAYER_TYPE_TRANSFORM;
    }
    if (lv_obj_get_style_transform_skew_x(obj, 0) != 0) {
        return LV_LAYER_TYPE_TRANSFORM;
    }
    if (lv_obj_get_style_transform_skew_y(obj, 0) != 0) {
        return LV_LAYER_TYPE_TRANSFORM;
    }
    if (lv_obj_get_style_opa_layered(obj, 0) != LV_OPA_COVER) {
        return LV_LAYER_TYPE_SIMPLE;
    }
    if (lv_obj_get_style_bitmap_mask_src(obj, 0) != NULL) {
        return LV_LAYER_TYPE_SIMPLE;
    }
    if (lv_obj_get_style_blend_mode(obj, 0) != LV_BLEND_MODE_NORMAL) {
        return LV_LAYER_TYPE_SIMPLE;
    }
...
static void full_cache_refresh(lv_obj_t *obj, lv_part_t part)
{
    ...
    if (part == LV_PART_MAIN || part == LV_PART_ANY) {
        ...
        for (i = 0; i < obj->style_cnt; i++) {
            if (lv_obj_style_get_selector_part(obj->styles[i].selector) != LV_PART_MAIN) {
                continue;
            }
