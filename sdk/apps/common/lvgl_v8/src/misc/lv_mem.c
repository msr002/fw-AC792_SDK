...
#if LV_USE_MEM_MONITOR || LV_USE_MEM_MONITOR_JUST_PRINTF
...
void *lv_mem_alloc(size_t size)
{
    ...
    if (alloc) {
#if (LV_USE_MEM_MONITOR || LV_USE_MEM_MONITOR_JUST_PRINTF) && LV_MEM_CUSTOM == 0
        cur_used += size;
...
#if LV_MEM_CUSTOM == 0
    if ((uint32_t)data < (uint32_t)work_mem_int || (uint32_t)data >= (uint32_t)work_mem_int + sizeof(work_mem_int)) {
        free(data);
        return;
    }
...
#if LV_MEM_CUSTOM == 0
    void *new_p;
    if ((uint32_t)data_p < (uint32_t)work_mem_int || (uint32_t)data_p >= (uint32_t)work_mem_int + sizeof(work_mem_int)) {
        return realloc(data_p, new_size);
    } else {
#if LV_USE_MEM_MONITOR || LV_USE_MEM_MONITOR_JUST_PRINTF
        size_t old_size = 0;
        if (data_p) {
            //old_size = lv_tlsf_block_size(data_p); //取出来的size还不准,导致后续计算为负数,FIX_ME
        }
#endif
        new_p = lv_tlsf_realloc(tlsf, data_p, new_size);

        if (new_p) {
#if 0// LV_USE_MEM_MONITOR || LV_USE_MEM_MONITOR_JUST_PRINTF
            cur_used += new_size;
            cur_used -= old_size;
            max_used = LV_MAX(cur_used, max_used);
#endif
        } else {
            LV_LOG_ERROR("couldn't realloc memory (%lu bytes)", (unsigned long)new_size);
            lv_mem_free(data_p);
...
void lv_mem_monitor(lv_mem_monitor_t *mon_p)
{
    ...
    static int timeout_hdl;
    if (time_lapse(&timeout_hdl, 30000)) {
        printf("[LV_MEM] total_size:%3dKB, cur_used:%3dKB, max_used:%3dKB, free_cnt/used_cnt:%3d/%3d, frag_pct:%2d%%, biggest_free_size:%3dKB \r\n",
               LV_MEM_SIZE / 1024, (mon_p->total_size - mon_p->free_size) / 1024, max_used / 1024, mon_p->free_cnt, mon_p->used_cnt, mon_p->frag_pct, mon_p->free_biggest_size / 1024);
    }
#endif
