...
#elif LV_USE_MEM_MONITOR_JUST_PRINTF && LV_MEM_CUSTOM == 0
    lv_mem_monitor_t mon;
    lv_mem_monitor(&mon);
#endif
...
static void refr_sync_areas(void)
{
    ...
    if (_lv_ll_is_empty(&disp_refr->sync_areas)) {
        return;
    }
...
void refr_obj(lv_draw_ctx_t *draw_ctx, lv_obj_t *obj)
{
    ...
    if (layer_type == LV_LAYER_TYPE_NONE) {
        ...
        if (layer_type == LV_LAYER_TYPE_SIMPLE) {
            flags |= LV_DRAW_LAYER_FLAG_CAN_SUBDIVIDE;
        }
    ...
    if (drv->rotated == LV_DISP_ROT_180) {
        ...
        while (row < area_h) {
            ...
            if ((row == 0) && (area_h >= area_w)) {
                ...
                if (rot_buf == NULL) {
                    rot_buf = lv_mem_buf_get(LV_DISP_ROT_MAX_BUF);
                }
