...
static void scr_load_internal(lv_obj_t *scr)
{
    ...
    if (d->act_scr) {
        lv_event_send(old_scr, LV_EVENT_SCREEN_UNLOAD_START, NULL);
    }
    if (d->act_scr) {
        lv_event_send(scr, LV_EVENT_SCREEN_LOAD_START, NULL);
    }
    ...
    if (d->act_scr) {
        lv_event_send(old_scr, LV_EVENT_SCREEN_UNLOADED, NULL);
    }
