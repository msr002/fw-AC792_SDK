...
lv_res_t lv_img_decoder_open(lv_img_decoder_dsc_t *dsc, const void *src, lv_color_t color, int32_t frame_id)
{
    ...
    if (src_type == LV_IMG_SRC_VARIABLE) {
        ...
        src_dsc = lv_mem_alloc(sizeof(lv_img_dsc_t));

        if (lv_get_img_dsc_bin(src, src_dsc) != LV_RES_OK) {
            LV_LOG_ERROR("[lv_img_decoder_open]:lv_get_img_dsc_bin error!!!");
            return LV_RES_INV;
        }

#if LV_USE_RLE
        compress_type = lv_get_compress_type(src);
#endif
    ...
    if (dsc->src_type == LV_IMG_SRC_FILE) {
        ...
        if (src_temp[0] == 'm' && (flash_src_use_malloc == 0)) {
            ...
            storage_src = LV_SRC_IN_UNKNOWN;
        }
        dsc->src = src_dsc;
        dsc->storage_src = storage_src;
    } else {
        dsc->src = src;
    }
    ...
    _LV_LL_READ(&LV_GC_ROOT(_lv_img_decoder_ll), decoder) {
        ...
        if (decoder->info_cb == NULL || decoder->open_cb == NULL) {
            continue;
        }
...
{
    lv_res_t res = LV_RES_INV;
    if (dsc->decoder->read_line_cb) {
        res = dsc->decoder->read_line_cb(dsc->decoder, dsc, x, y, len, buf);
    }
...
#if LV_USE_RLE
#if (LV_USE_GPU_RLE == 0)
        if (dsc->img_data != NULL && compress_type == LV_COMPRESS_RLE) {
...
lv_res_t lv_img_decoder_built_in_info(lv_img_decoder_t *decoder, const void *src, lv_img_header_t *header)
{
    ...
    if (src_type == LV_IMG_SRC_VARIABLE) {
        ...
        lv_img_dsc_t src_dsc;
        if (lv_get_img_dsc_bin_header(src, header, NULL) != LV_RES_OK) {
            return LV_RES_INV;
        }
...
#if LV_USE_GPU_JL_DMA2D || LV_USE_GPU_JL_GPU2P5D
            dsc->img_data = ((lv_img_dsc_t *)dsc->src)->data;
            return LV_RES_OK;
#endif
        }
