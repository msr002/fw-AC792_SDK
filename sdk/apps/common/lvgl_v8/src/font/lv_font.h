...
/** An enumeration value of the vertex path type.*/
typedef enum {
    LV_FONT_VERTEX_MOVE_TO = 1,
    LV_FONT_VERTEX_LINE_TO,
    LV_FONT_VERTEX_QUAD_TO,
    LV_FONT_VERTEX_CUBIC_TO,
} lv_font_path_type_t;

/** Describe the vertex and path of a font*/
typedef struct _lv_font_vertex_t {
    float x;        // x of the vertex coordinates
    float y;        // y of the vertex coordinates
    float cx1;      // Control point 1 coordinates x
    float cy1;      // Control point 1 coordinates y
    float cx2;      // Control point 2 coordinates x
    float cy2;      // Control point 2 coordinates x
    lv_font_path_type_t type;   // The path type of the vertex
} lv_font_vertex_t;

typedef struct _lv_font_path_t {
    lv_font_vertex_t *vertex_list;
    uint16_t vertex_number;
    uint32_t points_num;
} lv_font_path_t;

typedef struct _lv_font_jlgpu_privately_t {
    bool is_support;

    /** Gets the vertex path of the glyph*/
    const lv_font_path_t *(*get_glyph_vertex_path)(const struct _lv_font_t *, uint32_t);
} lv_font_jlgpu_privately_t;
 ...
 * @return pointer to the vertex path of the letter
 */
