...
static void awk_render_polyline_adapter(uint32_t map_id, awk_point_t *points, uint32_t point_size, const awk_paint_style_t *style)
{
    if (first_polyline) {
        ...
        printf("设置画\n");
        printf("开始画\n");
        printf("此时act的buf是：%p", lv_disp_get_draw_buf(_lv_refr_get_disp_refreshing())->buf_act);
#endif
        ...
        // 基于全屏buf区域绘制
        lv_area_t disp_area;
        lv_area_set(&disp_area, 0, 0, disp_hor - 1, disp_ver - 1);
        ...
        u8 *non_act_buf = (_lv_refr_get_disp_refreshing()->driver->draw_buf->buf1 == _lv_refr_get_disp_refreshing()->driver->draw_buf->buf_act) ? _lv_refr_get_disp_refreshing()->driver->draw_buf->buf2 : _lv_refr_get_disp_refreshing()->driver->draw_buf->buf1;
        u8 *act_buf = _lv_refr_get_disp_refreshing()->driver->draw_buf->buf_act;
        // 遍历点对，绘制折线
        for (uint32_t i = 0; i < point_size - 1; i++) {
            lv_point_t p1 = { .x = points[i].x, .y = points[i].y };
            lv_point_t p2 = { .x = points[i + 1].x, .y = points[i + 1].y };
            lv_draw_line(draw_ctx, &line_dsc, &p1, &p2);
        }
        INFO_PRINTF("buf1:%p buf2:%p act_buf:%p\n", _lv_refr_get_disp_refreshing()->driver->draw_buf->buf1, _lv_refr_get_disp_refreshing()->driver->draw_buf->buf2, _lv_refr_get_disp_refreshing()->driver->draw_buf->buf_act);
        BUF_DMA2D_COPY(
            NULL, // draw_ctx 可传NULL，如果没用上下文的话
            act_buf,
            disp_hor, // 目标 stride 宽度（单位像素）
        & (lv_area_t) {
            0, 0, disp_hor - 1, disp_ver - 1
        },
        non_act_buf,
        disp_hor,
        &(lv_area_t) {
...
static void awk_render_bitmap_adapter(uint32_t map_id, awk_rect_area_t area, awk_bitmap_t bitmap, const awk_paint_style_t *style)
{
    ...
    if (first_polyline) {
        first_polyline = 0;
        linenum = 0;
        line_obj = lv_img_create(lv_scr_act());
        lv_obj_set_pos(line_obj, 0, 0);
        lv_obj_set_size(line_obj, map_hor, map_ver);
        //下载网格图片
        char *temp = (char *)malloc(150);
        strcpy(temp, "storage/sd0/C/tempdata/gird.bin");
        FILE *fp = fopen("storage/sd0/C/tempdata/gird.bin", "w+");
        uint8_t *temp_buffer = malloc(map_ver * map_hor * 2);
        BUF_DMA2D_COPY(
            NULL, // draw_ctx 可传NULL，如果没用上下文的话
            temp_buffer,
            map_hor, // 目标 stride 宽度（单位像素）
        & (lv_area_t) {
            0, 0, map_hor - 1, map_ver - 1
        },
        _lv_refr_get_disp_refreshing()->driver->draw_buf->buf_act,
        disp_hor,
        &(lv_area_t) {
            0, 0, map_hor - 1, map_ver - 1
        });
        if (fp) {
            size_t written = fwrite(temp_buffer, 1, map_ver * map_hor * 2, fp);
            fclose(fp);
            INFO_PRINTF("💾 已保存原始图像数据 %d 字节 到 %s\n", written, temp);
        } else {
            INFO_PRINTF("❌ 打开文件失败：%s\n", temp);
        }
    }
    if (mapi == 0) {
#if MAP_BORDER_GIRD_FIX
        FILE *fp = fopen("storage/sd0/C/tempdata/gird.bin", "rb");
        uint8_t *fp_buffer = _lv_refr_get_disp_refreshing()->driver->draw_buf->buf_act;
        if (fp) {
            for (int i = 0 ; i < map_ver; i++) {
                fread(fp_buffer + i * disp_hor * 2, 1, map_hor * 2, fp);
            }
            fclose(fp);
        }
#endif
    ...
    lv_draw_jl_dma2d_buffer_copy(
        NULL, // draw_ctx 可传NULL，如果没用上下文的话
        lv_disp_get_draw_buf(_lv_refr_get_disp_refreshing())->buf_act,
        disp_hor, // 目标 stride 宽度（单位像素）
    & (lv_area_t) {
        area.x, area.y, area.x + bitmap.width - 1, area.y + bitmap.height - 1
    },
    bitmap.buffer,
    bitmap.width,
    &(lv_area_t) {
        0, 0, bitmap.width - 1, bitmap.height - 1
    });  //dma2d_buffer_copy
    // 设置显示位置和大小
...
static void awk_render_color_adapter(uint32_t map_id, awk_rect_area_t area, const awk_paint_style_t *style)
{
    ...
    INFO_PRINTF("🖌️ render color start for map_id=%d, area=(%d,%d,%d,%d), color=0x%08X\n",
                map_id, area.x, area.y, area.width, area.height, style->color);
    ...
    // 获取当前显存指针和 stride
    lv_disp_t *disp = _lv_refr_get_disp_refreshing();
    lv_color_t *dest_buf1 = (lv_color_t *)disp->driver->draw_buf->buf1;
    lv_color_t *dest_buf2 = (lv_color_t *)disp->driver->draw_buf->buf2;
    lv_coord_t stride = disp_hor;  // 一般是屏幕宽度

    // 调用DMA2D加速的颜色填充
    lv_draw_jl_dma2d_blend_fill(dest_buf1, stride, &lv_area, lv_color);
    JL_DMA2D_FILL(dest_buf2, stride, &lv_area, lv_color);
    INFO_PRINTF("✅ blend_fill done: (%d,%d)-(%d,%d), color R=%d G=%d B=%d\n",
                lv_area.x1, lv_area.y1, lv_area.x2, lv_area.y2, r, g, b);
}
