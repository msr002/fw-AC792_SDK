...
//HID
    ...
    HID_INTR_INTERVAL,          // Poll every 1msec seconds
...
u32 hid_desc_config(const usb_dev usb_id, u8 *ptr, u32 *cur_itf_num)
{
    ...
    log_info("hid interface num:%d", *cur_itf_num);
...
u32 hid_register(const usb_dev usb_id)
{
    ...
    if (hid_info[usb_id] == NULL) {
        ...
        if (!hid_info[usb_id]) {
            log_error("hid malloc hid_info[%d] fail", usb_id);
            return (u32) - 1;
        }
        hid_info[usb_id]->stream = (u8 *)malloc(CBUF_SIZE);
        if (!hid_info[usb_id]->stream) {
            log_error("hid malloc cbuffer space fail");
            err = (u32) - 1;
            goto __exit;
        }
