...
extern u32 jl_file_head_valid_check(JL_FILE_HEAD *head);
    ...
    if (usb_update && usb_update->updateFlag) {
        ...
        if (offset == (usb_update->firstSectorNum + total_sector_num - 1)) {
            if (calc_crc != usb_update->dataCrc) {
                log_error("[crc err] calc_crc : 0x%x, dataCrc : 0x%x", calc_crc, usb_update->dataCrc);
                goto __err_;
            }
