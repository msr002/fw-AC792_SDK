...
extern u32 usb_epbuf_free_space(void);
...
void *usb_h_alloc_ep_buffer(const usb_dev usb_id, u32 ep, u32 dma_size)
{
    ...
    if (ep_buffer == NULL) {
        log_info("\nep buffer free space:%d", usb_epbuf_free_space());
    }
#else
    ep_buffer = usb_h_get_ep_buffer(usb_id, ep);
#endif
    ASSERT(ep_buffer, "%s() ep_buffer not enough !!!, usb_id = %d, ep = %x, dma_size = %d\n", __func__, usb_id, ep, dma_size);
...
void usb_host_free(usb_dev usb_id)
{
    ...
    log_info("%s() %x", __func__, host_var[usb_id]);
