...
static volatile u8 usb_g_isr_in_flag[USB_MAX_HW_NUM];
...
#endif
    log_info("usb%d slave free ep buffer %x", usb_id, (u32)buf);
}
...
static void usb_isr(const usb_dev usb_id)
{
    u32 intr_usb, intr_usbe;
    u32 intr_tx, intr_txe;
    u32 intr_rx, intr_rxe;

    usb_g_isr_in_flag[usb_id] = 1;
    ...
    usb_g_isr_in_flag[usb_id] = 0;

    __asm__ volatile("csync");
}

static void usb_sof_isr(const usb_dev usb_id)
{
    struct usb_device_t *usb_device = usb_id2device(usb_id);
...
void usb_g_isr_wait_exit(usb_dev usb_id)
{
    if (!cpu_in_irq()) {
        while (usb_g_isr_in_flag[usb_id]);
    }
}

SET_INTERRUPT
static void usb0_g_isr(void)
{
    usb_isr(0);
}

SET_INTERRUPT
static void usb0_sof_isr(void)
{
    usb_sof_isr(0);
}
...
#if USB_MAX_HW_NUM == 2
SET_INTERRUPT
static void usb1_g_isr(void)
{
    usb_isr(1);
}

SET_INTERRUPT
static void usb1_sof_isr(void)
{
    usb_sof_isr(1);
}
#endif
...
u32 usb_config(const usb_dev usb_id)
{
    ...
    log_debug("zalloc: usb_config_var = %x", usb_config_var[usb_id]);
...
{
    log_debug("free zalloc: usb_config_var = %x", usb_id, usb_config_var[usb_id]);
...
#if USB_MALLOC_ENABLE
    if (usb_config_var[usb_id]) {
        log_debug("free: usb_config_var = %x", usb_config_var[usb_id]);
