...
#if USB_HOST_ASYNC
        for (int i = 0; i < 20; i++) {
            ret = usb_sem_pend(host_dev, 5); //wait isr
            if (ret == OS_TIMEOUT) {
                //检查usb是否掉线
                if (((usb_read_devctl(usb_id) & BIT(2)) == 0)) {
                    log_error("usb%d_offline\n", usb_id);
                    ret = -DEV_ERR_OFFLINE;
                    goto __exit;
                }
...
int usb_recv_descriptor(struct usb_host_device *host_dev, u16 value, void *data, u16 size)
{
    int err = 0;
    err = usb_control_msg(host_dev, USB_REQ_GET_DESCRIPTOR, USB_DIR_IN,
                          value, 0, data, size);
