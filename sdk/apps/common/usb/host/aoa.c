...
static void aoa_epin_isr(struct usb_host_device *host_dev, u32 ep)
{
    u8 buffer[64] = {0};
    usb_dev usb_id = host_device2id(host_dev);
    usb_h_ep_read_async(usb_id, ep, aoa.target_epin, buffer, sizeof(buffer), USB_ENDPOINT_XFER_BULK, 0);
    /* g_printf("RX:"); */
...
u32 aoa_process(u32 mode, u32 id)
{
    ...
    log_info("D(%d)->H(%d)", aoa.target_epin, aoa.host_epin);
    log_info("H(%d)->D(%d)",  aoa.host_epout, aoa.target_epout);
