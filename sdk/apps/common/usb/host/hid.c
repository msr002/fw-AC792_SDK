...
static struct hid_device_t hid_device[USB_MAX_HW_NUM][MAX_HOST_INTERFACE];
...
static const struct usb_interface_info _usb_if[USB_MAX_HW_NUM][MAX_HOST_INTERFACE] = {
    {
        ...
        {
            .ctrl = (struct interface_ctrl *) &hid_ctrl,
            .dev.hid = &hid_device[0][4],
        },
        {
            .ctrl = (struct interface_ctrl *) &hid_ctrl,
            .dev.hid = &hid_device[0][5],
        },
        {
            .ctrl = (struct interface_ctrl *) &hid_ctrl,
            .dev.hid = &hid_device[0][6],
        },
        {
            .ctrl = (struct interface_ctrl *) &hid_ctrl,
            .dev.hid = &hid_device[0][7],
        },
    },
#if USB_MAX_HW_NUM > 1
    {
        ...
        {
            .ctrl = (struct interface_ctrl *) &hid_ctrl,
            .dev.hid = &hid_device[1][4],
        },
        {
            .ctrl = (struct interface_ctrl *) &hid_ctrl,
            .dev.hid = &hid_device[1][5],
        },
        {
            .ctrl = (struct interface_ctrl *) &hid_ctrl,
            .dev.hid = &hid_device[1][6],
        },
        {
            .ctrl = (struct interface_ctrl *) &hid_ctrl,
            .dev.hid = &hid_device[1][7],
        },
    },
#endif
};
...
int usb_hid_parser(struct usb_host_device *host_dev, u8 interface_num, const u8 *pBuf)
{
    ...
    memset(usb_if->dev.hid, 0, sizeof(struct hid_device_t));

    host_dev->interface_info[interface_num] = usb_if;
    usb_if->dev.hid->parent = host_dev;

    log_info("hid eps %d  %d %x %x", interface->bNumEndpoints, interface_num, usb_if, usb_if->dev.hid);
...
{
    _hid_report_parse(hid, report, len);
...
static void hid_convert_krbd(const struct report_info_t *kbd, u8 *buffer)
{
#if 0
...
static void hid_route(const struct hid_device_t *hid, u8 *buffer)
{
    for (int i = 0; i < hid->report_count; i++) {
        if (hid->report_list[i].usage == 0x02) {
            ...
            log_info("usage %x", hid->report_list[i].usage);
        }
    }
}
...
{
    struct usb_host_device *host_dev = (struct usb_host_device *)host_id2device(id);
    u8 report[256 + 2];
    u8 ep_pair[4];
    ...
    for (u8 i = 0; i < MAX_HOST_INTERFACE; i++) {
        ...
        if (usb_if &&
            ...
            for (int n = 0; n < usb_if->dev.hid->bNumEndpoints; n++) {
                ...
                usb_h_set_ep_isr(host_dev, host_ep | USB_DIR_IN, (usb_h_interrupt)hid_isr, (void *)usb_if);
                ...
                usb_h_ep_read_async(id, host_ep, target_ep, NULL, 0, USB_ENDPOINT_XFER_INT, 1);
            }
        } else {
            if (usb_if) {
...
void hid_stop_process(u32 id)
{
    usb_h_set_ep_isr(NULL, 0, NULL, NULL);

    if (hid_intr_ep_in_buf[id]) {
        usb_h_free_ep_buffer(id, hid_intr_ep_in_buf[id]);
        hid_intr_ep_in_buf[id] = NULL;
    }
}
