...
{
    return (adc_get_voltage(ADC_PMU_CH_VBAT) * 4 / 10);
}
...
{
    u16 tmp_bat_val;
    u16 bat_value = get_vbat_level();

    if (battery_full_value == 0) {
        battery_full_value = 420;
    }

    if (bat_value <= TCFG_LOW_POWER_OFF_VAL) {
        return 0;
    }

    tmp_bat_val = remap_calculate_vbat_percent(bat_value);
    if (!tmp_bat_val) {
        tmp_bat_val = ((u32)bat_value - TCFG_LOW_POWER_OFF_VAL) * 100 / (battery_full_value - TCFG_LOW_POWER_OFF_VAL);
        if (tmp_bat_val > 100) {
            tmp_bat_val = 100;
        }
    }
...
{
    if (bat_val <= TCFG_LOW_POWER_SHUTDOWN) {
        return TRUE;
    }
    return FALSE;
}
...
void vbat_check_slow(void *priv)
{
    if (vbat_fast_timer == 0) {
        vbat_fast_timer = sys_s_hi_timer_add(NULL, vbat_check, 10);
    }
...
void vbat_check(void *priv)
{
    ...
    log_debug("bv:%d, bl:%d", bat_val, cur_battery_level);

    unit_cnt++;

    /* if (bat_val < LOW_POWER_OFF_VAL) { */
    if (bat_val <= TCFG_LOW_POWER_OFF_VAL) {
        low_off_cnt++;
    }
    /* if (bat_val < LOW_POWER_WARN_VAL) { */
    if (bat_val <= TCFG_LOW_POWER_WARN_VAL) {
        low_warn_cnt++;
    }
    ...
    if (unit_cnt >= VBAT_DETECT_CNT) {
        if (get_charge_online_flag() == 0) {
            if (low_off_cnt > (VBAT_DETECT_CNT / 2)) { //低电关机
                ...
                if (low_power_cnt > 6) {
                    log_info("*******Low Power,enter softpoweroff********");
                    low_power_cnt = 0;
                ...
                if ((low_voice_first_flag && low_voice_cnt > 1) || //第一次进低电10s后报一次
                    ...
                    low_voice_cnt = 0;
                    if (!lowpower_timer) {
                        log_info("**Low Power,Please Charge Soon!!!**");
...
int app_power_event_handler(int event)
{
    switch (event) {
    ...
    case POWER_EVENT_POWER_WARNING:
        if (lowpower_timer == 0) {
            lowpower_timer = sys_timer_add_to_task("sys_timer", (void *)POWER_EVENT_POWER_WARNING, (void (*)(void *))power_event_to_user, TCFG_LOW_POWER_WARN_TIME);
        }
        break;
    case POWER_EVENT_POWER_LOW:
        log_error("POWER_EVENT_POWER_LOW");
        vbat_timer_delete();
        if (lowpower_timer) {
            sys_timer_del(lowpower_timer);
            lowpower_timer = 0;
...
static void sys_power_auto_shutdown_schedule(void *priv)
{
    ...
    if (++auto_shdn_cnt >= time) {
        auto_shdn_cnt = 0;
        log_info("count down to 0 to shutdown");
...
SYS_EVENT_STATIC_HANDLER_REGISTER(sys_power_enent, 0) = {
...
void sys_power_init(void)
{
    ...
    if (get_vbat_level() <= TCFG_LOW_POWER_SHUTDOWN) {
        power_set_soft_poweroff(0);
    }
