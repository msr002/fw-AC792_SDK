...
void bt_name_set_control(u8 *name, u32 len)
{
    ...
    if (INTSR_TASK_SPI_API) {
        INTSR_TASK_SPI_API->custom_spi_send_cmd_handle(OP_CODE_BT_CONCTRL, 1, RSP_CMD_TYPE_REQUEST, data, len + 2);
    }
...
void bt_mac_set_control(u8 *mac, u32 len)
{
    ...
    if (INTSR_TASK_SPI_API) {
        INTSR_TASK_SPI_API->custom_spi_send_cmd_handle(OP_CODE_BT_CONCTRL, 1, RSP_CMD_TYPE_REQUEST, data, len + 1);
    }
...
void bt_shutdown_control(void)
{
    ...
    if (INTSR_TASK_SPI_API) {
        INTSR_TASK_SPI_API->custom_spi_send_cmd_handle(OP_CODE_BT_MCU_CONTROL, 1, RSP_CMD_TYPE_REQUEST, &data, sizeof(data));
    }
}
 ...
 * @param len 响应数据长度
 */
void bt_device_bt_rsp_control(u8 sn, SPI_ErrorCode cmd_status, u8 *rsp_data, u32 len)
{
    printf("%s %d\n", __func__, __LINE__);

    if (INTSR_TASK_SPI_API) {
        INTSR_TASK_SPI_API->custom_spi_response_cmd_handle(OP_CODE_BT_CONCTRL, sn, RSP_CMD_TYPE_RSP, cmd_status, rsp_data, len);
    }
}
 ...
 * @param len 响应数据长度
 */
void bt_device_mcu_rsp_control(u8 sn, SPI_ErrorCode cmd_status, u8 *rsp_data, u32 len)
{
    printf("%s %d\n", __func__, __LINE__);

    if (INTSR_TASK_SPI_API) {
        INTSR_TASK_SPI_API->custom_spi_response_cmd_handle(OP_CODE_BT_MCU_CONTROL, sn, RSP_CMD_TYPE_RSP, cmd_status, rsp_data, len);
    }
}
...
#define CHANNEL_DATA_SIZE   (BITARRAY_SIZE + 1)
    ...
    if (INTSR_TASK_SPI_API) {
        INTSR_TASK_SPI_API->custom_spi_send_cmd_handle(OP_CODE_BT_MCU_CONTROL, 1, RSP_CMD_TYPE_REQUEST, channel_info, CHANNEL_DATA_SIZE);
    }
