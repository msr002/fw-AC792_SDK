...
/*************************************************************************************************/
/**
 * @brief   SPI数据传输包解析错误码枚举
 *
 * @note    用于标识SPI数据接收和解码过程中的各类错误状态
 *************************************************************************************************/
typedef enum {
    SPI_ERR_SUCCESS      = 0x00, /**< 成功：操作顺利完成 */
    SPI_ERR_FAILED       = 0x01, /**< 失败：通用错误代码，表示未指明的操作失败 */
    SPI_ERR_UNDEFINED    = 0x02, /**< 未定义：遇到了未定义或未知的错误状态 */
    SPI_ERR_BUSY         = 0x03, /**< 忙碌状态：设备或资源正忙，无法立即响应请求 */
    SPI_ERR_NO_RESPONSE  = 0x04, /**< 没有收到回复：预期中的响应或确认信号未收到 */
    SPI_ERR_CRC          = 0x05, /**< CRC出错：整个数据包的循环冗余校验失败 */
    SPI_ERR_DATA_CRC     = 0x06, /**< 数据CRC出错：数据字段部分的CRC校验失败 */
    SPI_ERR_PARAM_OVERFLOW = 0x07, /**< 参数溢出：接收到的参数值超出允许范围 */
    SPI_ERR_DATA_OVERFLOW = 0x08, /**< 数据溢出：数据量超出缓冲区或预期长度 */
    SPI_ERR_LRC          = 0x09,  /**< LRC获取出错：纵向冗余校验(LRC)计算或验证失败 */
    SPI_ERR_LEN_ERROR     = 0x0A, /**< 长度错误：数据长度不符合预期 */
} SPI_ErrorCode;
