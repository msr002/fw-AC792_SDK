...
void bt_phone_get_pbap_info(void)
{
    ...
    if (INTSR_TASK_SPI_API) {
        INTSR_TASK_SPI_API->custom_spi_send_cmd_handle(OP_CODE_BT_PABP, 1, RSP_CMD_TYPE_REQUEST, &data, 1);
    }
}
...
void bt_phone_call_conctrl(u8 deal)
{
    ...
    if (INTSR_TASK_SPI_API) {
        INTSR_TASK_SPI_API->custom_spi_send_cmd_handle(OP_CODE_HFP_CALL_CONCTRL, 1, RSP_CMD_TYPE_REQUEST, &data, sizeof(data));
    }
}
...
void bt_phone_hfp_rsp_slave(uint8_t cmd_stats, u8 *data, u32 len)
{
    ...
    if (INTSR_TASK_SPI_API) {
        INTSR_TASK_SPI_API->custom_spi_response_cmd_handle(OP_CODE_HFP_CALL_CONCTRL, 1, BT_CMD_TYPE_REQUEST_WITH_RSP, cmd_stats, data, len);
    }
}
