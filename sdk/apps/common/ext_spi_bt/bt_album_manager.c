...
#if (INSTR_DEV_SPI_ENABLE && TCFG_SPI_ALBUM_PARSER_ENBALE)
/* 常量定义 */
...
static int album_save_to_sd(const u8 *jpg_data, u32 jpg_len)
{
    ...
    if (!jpg_data || jpg_len == 0) {
        ...
        return SPI_ERR_LEN_ERROR;
    }

    /* 存储设备检查 */
    if (!storage_device_ready()) {
        log_error("[%s] Storage device not ready\n", __func__);
        return SPI_ERR_FAILED;
    }
...
static void album_handle_end_transfer(u16 crc_value, u32 recv_end_len)
{
    u16 calculated_crc = 0;
    u32 transfer_time = 0;

    log_debug("[%s] End transfer, received: %u/%u\n",
              __func__, g_album_mgr.cur_recv_len, g_album_mgr.jpg_total_len);

    /* 长度检查 */
    if (g_album_mgr.cur_recv_len != g_album_mgr.jpg_total_len && g_album_mgr.cur_recv_len != recv_end_len) {
        log_error("[%s] Length mismatch: expected=%u, recv_end_len=%u, actual=%u\n",
                  __func__, g_album_mgr.jpg_total_len, recv_end_len, g_album_mgr.cur_recv_len);
...
static void album_process_message(const bt_album_msg_t *msg)
{
    ...
    case PROTOCOL_TAG_BT_ALBUM_TRANSFER_DATA_END:
        album_handle_end_transfer(msg->crc_value, msg->end_data_len);
        break;
    default:
...
static void album_recv_task(void *priv)
{
    ...
    while (1) {
        ...
        album_msg.end_data_len = msg[5];   // end_data_len值在msg[5]中
        /* 处理消息 */
