...
//ptr:usb数据指针
//len:发送的数据长度
int usb_audio_record_get_buf(const usb_dev usb_id, void *ptr, u32 len, u8 channel, u32 sample_rate)
{
    xc_talkback_read(ptr, len);
    return len;
}
static int usb_host_audio_test_init(void)
{
    usb_host_audio_init(0, usb_audio_play_put_buf, usb_audio_record_get_buf);
#if USB_MAX_HW_NUM > 1
    usb_host_audio_init(1, usb_audio_play_put_buf, usb_audio_record_get_buf);
#endif
    return 0;
}
