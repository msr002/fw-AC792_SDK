...
OS_MUTEX get_cloud_playback_list_mutex()
{
    return cloud_playback_list_mutex;
}

bp_db_t get_db_element(int index)
{
    if (index < 0 || index >= 3) {
        log_error("db index is out of range!");
        ASSERT(0);
    }
