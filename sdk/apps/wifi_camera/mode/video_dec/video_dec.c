...
static int video_dec_key_event_handler(struct key_event *key)
{
    ...
    case KEY_EVENT_CLICK:
        ...
        case KEY_OK:
            printf("key ok");
            app_send_message(APP_MSG_DEC_PAUSE, 0);
            break;
        case KEY_UP:
            printf("key up");
            app_send_message(APP_MSG_DEC_FR, 0);
            break;
        case KEY_DOWN:
            printf("key down");
            app_send_message(APP_MSG_DEC_FF, 0);
            break;
        case KEY_MODE:
            printf("key mode");
            app_send_message(APP_MSG_DEC_FILE, 0);
            break;
        default:
            break;
        }
    ...
    case KEY_EVENT_LONG:
        switch (key->value) {
        case KEY_UP:
            app_send_message(APP_MSG_DEC_SCAN_DOWN, 0);
            break;
        case KEY_DOWN:
            app_send_message(APP_MSG_DEC_SCAN_UP, 0);
            break;
            /* case 7: */
...
static int msg_handler(struct application *app, int *msg)
{
    ...
    case APP_MSG_DEC_FILE:
        ...
        break;
    case APP_MSG_DEC_SCAN_DOWN:
        printf("\n -[function] %s -[line] %d\n", __FUNCTION__, __LINE__);
        video_dec_stop(0);
        __this->curr_dir++;
        video_dec_scan_dir();
        break;
    case APP_MSG_DEC_SCAN_UP:
        printf("\n -[function] %s -[line] %d\n", __FUNCTION__, __LINE__);
        video_dec_stop(0);
        __this->curr_dir--;
        video_dec_scan_dir();
        break;
    case APP_MSG_DEC_MAIN:
        printf("\n -[function] %s -[line] %d\n", __FUNCTION__, __LINE__);
#ifndef CONFIG_FILE_PREVIEW_ENABLE
        /*
...
#endif
        break;
    case APP_MSG_DEC_CHANGE_STATUS:
        it = (struct intent *)msg[1];
        /* it = (struct intent*)(*(int*)(msg[1])); */
        printf("\n -[function] %s -[line] %d\n", __FUNCTION__, __LINE__);
        ret = video_dec_change_status(__this, it);
        free(it);
        it = NULL;
        break;
    case APP_MSG_DEC_SET_CONFIG:
        /* it = (struct intent*)(*(int*)(msg[1])); */
        it = (struct intent *)msg[1];
        printf("\n -[function] %s -[line] %d\n", __FUNCTION__, __LINE__);
        /*if (!__this->ui) {*/
        ...
        break;
#ifdef CONFIG_FILE_PREVIEW_ENABLE
    case APP_MSG_DEC_OPEN_FILE:
        it = (struct intent *)msg[1];
        printf("it:%p, ex data:%p, msg:%p", it, it->exdata, msg[1]);
        printf("\n -[function] %s -[line] %d\n", __FUNCTION__, __LINE__);
        if (!__this->req.dec.file) {
            printf("\n -[function] %s -[line] %d\n", __FUNCTION__, __LINE__);
            __this->req.dec.file = (FILE *)it->data;
        }
        printf("fd:%p, path:%s", it->data, it->exdata);
        strcpy(__this->cur_path, (char *)it->exdata);
        dec_open_file();
        free(it);
        it = NULL;
        break;
    case APP_MSG_DEC_PREV_NEXT_CONTROL:
        printf("\n -[function] %s -[line] %d\n", __FUNCTION__, __LINE__);
        it = (struct intent *)msg[1];
        int *ptr = (int *)msg[1];
        printf("ptr:%p", ptr);
        ...
        it = NULL;
        break;
    case APP_MSG_DEC_CONTROL:
        printf("\n -[function] %s -[line] %d\n", __FUNCTION__, __LINE__);
        video_dec_play_pause();
        break;
    case APP_MSG_DEC_GET_FRAME:
        printf("\n -[function] %s -[line] %d\n", __FUNCTION__, __LINE__);
        video_dec_get_frame();
        break;
    case APP_MSG_DEC_SET_AUDIO_VOLUME:
        printf("\n -[function] %s -[line] %d\n", __FUNCTION__, __LINE__);
        video_dec_switch_audio_volume();
        break;
    case APP_MSG_DEC_SET_PLAY_SPEED:
        printf("\n -[function] %s -[line] %d\n", __FUNCTION__, __LINE__);
        it = (struct intent *)msg[1];
        /* it = (struct intent*)(*(int*)(msg[1])); */
        printf("\n -[function] %s -[line] %d\n", __FUNCTION__, __LINE__);
