...
static void phone_call_send_cmd(u8 cmd, u8 *btaddr, u8 bt_value);
...
static int bt_phone_income(u8 after_conn, u8 *bt_addr)
{
    ...
    log_info("inband_ringtone=0x%x %d", __this->inband_ringtone, after_conn);
...
#define PHONE_CALL_AEC_DATA_TO_SD 0
    ...
    if (addr && bt_get_call_status_for_addr(bt_addr) != BT_CALL_OUTGOING) {
        ...
        if (call_status == BT_CALL_INCOMING) {
            ...
            if ((inband_ring_flag == 0) || (inband_ring_flag && bt_check_esco_state_via_addr(__this->phone_ring_addr) == BT_ESCO_STATUS_CLOSE)) {
                phone_call_send_cmd(CMD_PHONE_INCOME, __this->phone_ring_addr, 2);
            }
        }
    }

#if PHONE_CALL_AEC_DATA_TO_SD
...
static void bt_phone_sync_vol(void)
{
    bt_cmd_prepare(USER_CTRL_HFP_CALL_SET_VOLUME, 1, &__this->phone_vol);
}

static void phone_call_send_cmd(u8 cmd, u8 *bt_addr, u8 bt_value)
{
    switch (cmd) {
...
static int phone_income_num_play_end(void *priv, enum stream_event ev)
{
    switch (ev) {
...
static void phone_income_num_check(void *priv)
{
    ...
    if (__this->phone_num_flag) {
        if (__this->phone_ring_flag) {
            ...
            if (__this->inband_ringtone == 0) {
                int ret = play_tone_files_alone_callback(file_list, file_num, NULL, phone_income_num_play_end);
                if (ret) {
                    phone_income_num_play_end(NULL, STREAM_EVENT_STOP);
                }
            } else {
...
static int bt_phone_status_event_handler(void *event)
{
...
#endif
        phone_call_send_cmd(CMD_PHONE_HANGUP, bt->args, 0);
        break;
    ...
    case BT_STATUS_SCO_STATUS_CHANGE:
        ...
        if (bt->value != 0xff) {
...
#endif
            phone_call_send_cmd(CMD_OPEN_ESCO_PLAYER, bt->args, 0);
        } else {
            u8 bt_esco_play[6];
            ...
            phone_call_send_cmd(CMD_CLOSE_ESCO_PLAYER, bt->args, 0);
#if TCFG_MIC_EFFECT_ENABLE
    ...
    case BT_STATUS_LAST_CALL_TYPE_CHANGE:
        ...
        phone_call_send_cmd(CMD_PHONE_LASET_CALL_TYPE, bt->args, bt->value);
        break;
    ...
    case BT_STATUS_SIRI_GET_STATE:
        ...
        esco_dump_packet = ESCO_DUMP_PACKET_DEFAULT;
        phone_call_send_cmd(CMD_PHONE_SIRI, bt->args, bt->value);
        break;
    default:
        break;
    }
