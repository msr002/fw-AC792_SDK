...
void parking_page_show(int arg)
{
    ...
    if (video_rec_scr->video_rec_del != false) {
        ...
        if (screen == NULL) {
            screen = gui_scr_create(GUI_SCREEN_VIDEO_REC, "video_rec", video_rec_scr->video_rec, (gui_scr_setup_cb_t)setup_scr_video_rec, (gui_scr_unload_cb_t)unload_scr_video_rec);
        }
