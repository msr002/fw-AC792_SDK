...
static int __net_music_dec_file(void *file)
{
    ...
    log_info("url_file_type: %s", req.dec.dec_type);
...
__err:
    log_error("play_net_music_faild");
...
int net_music_dec_file(const char *url)
{
    ...
    if (err) {
        log_error("net_download_open: err = %d", err);
        return err;
    }
...
//切换上一首或下一首
int net_music_dec_switch_file(int fsel_mode)
{
    union ai_req req = {0};
...
//获取断点数据
int net_music_get_dec_breakpoint(struct audio_dec_breakpoint *bp)
{
    int err;
    union audio_req r = {0};
...
//设置音量大小
int net_music_set_dec_volume(int step)
{
    union audio_req req = {0};
    union ai_req ai = {0};
...
//获取解码器状态
int net_music_get_dec_status(void)
{
    union audio_req req = {0};
...
int net_music_dec_seek(int seek_step)
{
    union audio_req r = {0};
