...
int recorder_play_to_dac(int sample_rate, u8 channel, const char *format)
{
    int err;
    union audio_req req = {0};
...
static int recorder_key_click(struct key_event *key)
{
    ...
    switch (key->value) {
    case KEY_OK:
#if 0//CONFIG_AUDIO_ENC_SAMPLE_SOURCE == AUDIO_ENC_SAMPLE_SOURCE_LINEIN || CONFIG_AUDIO_ENC_SAMPLE_SOURCE == AUDIO_ENC_SAMPLE_SOURCE_MIC
        if (__this->direct) {
            if (__this->run_flag) {
                recorder_close();
            } else {
