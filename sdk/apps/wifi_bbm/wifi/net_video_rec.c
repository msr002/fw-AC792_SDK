...
static struct video_rec_hdl rec_handler;
...
static int net_video_rec0_start()
{
    ...
    //间隔越大延时越大
    req.rec.audio.aud_interval_size = 640;
...
static int net_video_rec0_stop(u8 close)
{
    ...
    if (__this_net->net_video_rec) {
        ...
        net_video_rec_set_bitrate(net_video_rec_get_abr(__this_net->net_videoreq[0].rec.width));
        if (close) {
...
static int net_video_rec1_stop(u8 close)
{
    ...
    if (__this_net->net_video_rec1) {
        ...
        net_video_rec_set_bitrate(net_video_rec_get_abr(__this_net->net_videoreq[1].rec.width));
