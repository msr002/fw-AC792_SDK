...
#define ENABLE_BBM_AUDIO_TEST 1

#if ENABLE_BBM_AUDIO_TEST
...
#define CONN_PORT 9981  //接收数据端口

#define AUDIO_DEC_BUF_MAX_LEN 2*1024

static u8 *recv_buf;
static u32 recv_buf_len;
static void *ctp_recv_sockfd;
static int ctp_recv_pid;
static u8 task_exit;
...
static void net_ctp_recv_task(void)
{
    ...
    sock_set_recv_timeout(ctp_recv_sockfd, 1000);
    ...
    while (1) {
        ...
        recv_len = sock_recvfrom(ctp_recv_sockfd, recv_buf, recv_buf_len, 0, NULL, NULL);
        if (recv_len <= 0) {
            printf("recv err \n");
            continue;
        }
...
static int net_ctp_recv_init(void)
{
    printf("\n\n-----------net_ctp_recv_init----------\n\n");

    if (ctp_recv_sockfd) {
        printf("err ctp recv sockfd Initialized \n");
        return -1;
    }
    ...
    ctp_recv_sockfd = sock_reg(AF_INET, SOCK_DGRAM, 0, NULL, NULL);
    if (ctp_recv_sockfd == NULL) {
        printf("sock_reg err\n");
        return -1;
    }

    ret = sock_set_reuseaddr(ctp_recv_sockfd);
    if (ret) {
        printf("sock_set_reuseaddr err:%d\n", ret);
        return -1;
    }

    ret = sock_bind(ctp_recv_sockfd, (struct sockaddr *)&conn_addr, sizeof(struct sockaddr));
    if (ret) {
        printf("sock_bind err:%d\n", ret);
        return -1;
    }

    recv_buf_len = 200 * 1024;
    ...
    thread_fork("thread_socket_recv", 25, 2048, 2048, &ctp_recv_pid, net_ctp_recv_task, NULL);

    return 0;
}

static int net_ctp_recv_exit(void)
{
    printf("\n\n-----------net_ctp_recv_exit----------\n\n");

    if (ctp_recv_pid) {
        task_exit = 1;
        thread_kill(&ctp_recv_pid, KILL_WAIT);
        task_exit = 0;
    }
    ...
    if (ctp_recv_sockfd) {
        sock_unreg(ctp_recv_sockfd);
        ctp_recv_sockfd = NULL;
    }

    return 0;
}

#endif /* ENABLE_BBM_AUDIO_TEST */
...
int cmd_put_video_param(void *priv,  char *content)
{
    ...
    //ios软件版本1.3.8测试宽高是反的
    if (strstr(height, "640")) {
        db_update("res", VIDEO_RES_VGA);
    } else if (strstr(height, "1280")) {
...
int cmd_put_format(void *priv, char *content)
{
    ...
    int format_err = sdcard_storage_device_format(SDX_DEV);
    if (format_err != 0) {
...
static int cmd_put_net_ota(void *priv, char *content)
{
    json_object *new_obj = NULL;
    json_object *parm = NULL;
    json_object *status = NULL;
    ...
    if (val) {
        printf("cmd_put_net_ota\n");
#if 0 //TODO
        int net_ota_single_back_init(void);
        if (net_ota_single_back_init()) {
            val = 0;
            printf("cmd_put_net_ota err\n\n");
        }
#endif
    }
    snprintf(buf, sizeof(buf), "status:%d", val);
    CTP_CMD_COMBINED(NULL, CTP_NO_ERR, "NET_OTA_SINGLE", "NOTIFY", buf);
#else
    strcpy(buf, "status:0");
    CTP_CMD_COMBINED(NULL, CTP_NO_ERR, "NET_OTA_SINGLE", "NOTIFY", buf);
#endif
...
const struct ctp_map_entry ctp_system_cmd_tab[] SEC_USED(.ctp_system_cmd) = {
    ...
    */
    ...
    {NULL, "NET_OTA_SINGLE", NULL, cmd_put_net_ota},
};
