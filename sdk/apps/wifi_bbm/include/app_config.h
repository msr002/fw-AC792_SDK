...
// WIFI RAW+AP 模式共存时。demo宏
// 调试使用
// 使用说明:先关闭wifi_app_task.c中的wifi初始化流程。
// 在开启CONFIG_BBM_TX宏的前提下，分别开启下面两个宏,编译烧入两台设备
// 手机连接AP后，手机打开DVrunning APP开启图传。 同时打开 屏幕端设备，即可同时显示。
// 双网卡: 图传端实现了双网卡，AP网卡和手机通讯，RAW网卡和图传端设备通讯。
// 注意AP模式是读取wifi_conf.c设置的MAC地址，demo初始化中配置的MAC地址要一致
// #define WIFI_RAW_RX_DEV                  //屏幕端
// #define WIFI_RAW_TX_DEV                 //图传端
