...
#pragma bss_seg(".music_id3.data.bss")
#pragma data_seg(".music_id3.data")
#pragma const_seg(".music_id3.text.const")
#pragma code_seg(".music_id3.text")
#endif
...
#define FRAME_DAT_MAX		512
    ...
    while (addr < addr_end) {
        ...
        fread(frame_hdl, ID3_V2_FRAME_SIZE, 1, file);
        ...
        if (frame_len <= FRAME_DAT_MAX) {
            ...
            fread(frame_dat, frame_len, 1, file);
        }
        ...
        if (!memcmp(frame->frameID, "TPE1", 4)) {
            frame_dat++;
            sprintf((char *)tpe1_data, "%s", frame_dat);
...
MP3_ID3_OBJ *id3_v1_obj_get(void *file)
{
    ...
    fread(obj->id3_buf, ID3_V1_HEADER_SIZE, 1, file);
...
MP3_ID3_OBJ *id3_v2_obj_get(void *file)
{
    ...
    fread(tmp_buf, ID3_V2_HEADER_SIZE, 1, file);
    ...
    if (id3_v2_match(tmp_buf) == true) {
        ...
        fseek(file, 0, SEEK_SET);
        fread(obj->id3_buf, obj->id3_len, 1, file);
