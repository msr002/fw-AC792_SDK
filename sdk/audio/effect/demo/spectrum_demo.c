...
void user_spectrum_get_param()
{
    ...
    char *node_name       = "Spectrum";     //节点名称（节点内的第一参数，用户自定义,长度小于等于15byte）
...
void user_spectrum_advance_get_param(void)
{
    struct spectrum_advance_parm cfg = {0};
    const char *node_name = "SpectrumAdv";     //节点名称（节点内的第一参数，用户自定义,长度小于等于15byte）
...
void user_spectrum_advance_set_bypass(int bypass)
{
    spectrum_advance_param_tool_set cfg = {0};
    cfg.is_bypass = bypass;
    const char *node_name = "SpectrumAdv";
    jlstream_set_node_param(NODE_UUID_SPECTRUM_ADVANCE, node_name, &cfg, sizeof(cfg));
}
