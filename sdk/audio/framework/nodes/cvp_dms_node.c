...
static void cvp_node_dms_beamfroming_cfg_update(struct cvp_dms_beamfroming_cfg_t *cfg, void *priv)
{
    ...
    if (g_cvp_hdl) {
        ...
        log_info("talk_mic %d, talk_ref_mic %d, ref mic en %d, ref_mic_ch %d", g_cvp_hdl->mic_sel.talk_mic, g_cvp_hdl->mic_sel.talk_ref_mic, g_cvp_hdl->ref_mic.en, g_cvp_hdl->ref_mic.ref_mic_ch);
    }
...
static void cvp_node_dms_flexible_cfg_update(struct cvp_dms_flexible_cfg_t *cfg, void *priv)
{
...
static void cvp_node_dms_awn_cfg_update(struct cvp_dms_awn_cfg_t *cfg, void *priv)
{
...
static void cvp_node_dms_hybrid_cfg_update(struct cvp_dms_hybrid_cfg_t *cfg, void *priv)
{
...
#endif
    if (len != sizeof(ncfg)) {
        log_error("cvp_dms_node read ncfg err");
        return 0;
    }
